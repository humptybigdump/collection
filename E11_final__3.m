...
%% Exercise 11: Water movement in unsaturated soils
...
alpha = 14.50; % air entry values
n_vg = 2.68; % shape value
ks = 8.25e-5;% saturated hydraulic conductivity
...
theta(ip) = [theta_topsoil : -d_theta : theta_subsoil+d_theta]; % for the array positions of the upper soil, the theta vaöues are overwritten with the respective theta of upper soil

% Preallocation of arrays
psi = zeros(dim,1); % matric potential at each grid node
k = zeros(dim,1); % soil hydraulic conductivity at each grid node
q = zeros(dim-1,1); % soil water flux after Dary-Buckingham law
...
%% Main routine (solving Darcy-Buckingham law and plotting)
figure;
...
% Calculates Darcy-Buckingham flux between grid nodes with arithmetic mean, Eq. 1
for i = 1:dim-1 
    q(i) = -0.5 * (k(i) + k(i+1)) * ((psi(i+1) - psi(i)) / dz(1) + 1);  % Darcy-Buckingham flux based on the aritmetric mean    
end

% Updates soil water content/soil moisture profile based on water fluxes between nodes (constant conditions(flux at upper boundary, mimics
% a never ending water supply/infiltration at top of the soil domain; also constant condition at lower boundary, no outflow)
for i = 2:dim-1
