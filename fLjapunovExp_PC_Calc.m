function [vT,xLambda,vT_ges,xX_ges,vT_event,xX_event] = fLjapunovExp_PC_Calc(fODE,vParams, fSolver, log, sODE_options,vX0,nT0,nT1,nT_f);
  %% Beschreibung
  ...
  for nI = 1:nR
    ...
    [vT_i,xX_i,vT_i_event,xX_i_event] = feval(fSolver, @(t,x) fODE(t,x,vParams), [nT0_i nT1_i], [vX0_i;xY(:)], sODE_options);
    vT_ges = [vT_ges;vT_i];
    xX_ges = [xX_ges;xX_i(:,1:nN)];
    if(~isempty(vT_i_event))
      vT_event = [vT_event;vT_i_event];
      xX_event = [xX_event;xX_i_event(:,1:nN)];
    end
