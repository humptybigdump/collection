%STUD_MATLAB_INTRO_PART1 Einführung in Matlab/Simulink.
%
%  Description:
%          Einführung in Matlab/Simulink
%          Dieses Musterfile dient zum Erlernen der grundlegenden
%          Matlabbefehle. Es handelt sich dabei um ein sogenanntes
%          m-file, in welchem der Code zu einer Datei zusammengefasst
%          und direkt ausgeführt werden kann. Neben
%          diesem m-file kann der Code auch direkt im Command Window hinter
%          der  Eingabeaufforderung ">>" eingegeben und ausgeführt werden.
%
%          Das Prozentzeichen dient in Matlab zum Einbinden von Kommentaren.
%          Weiterhin ist es möglich durch zwei aufeinanderfolgende
%          Prozentzeichen den Quelltext in Abschnitte zu unterteilen. Das
%          Ausführen der Datei erfolgt entweder durch Eingabe des Dateinamens
%          im Command Window oder durch Drücken der F5-Taste, einzelne
%          Abschnitte können durch die Kombination STRG+Eingabe ausgeführt
%          werden. Einzelne Zeilen im Programmcode können weiterhin markiert
%          und durch Drücken der F9-Taste ausgeführt werden.
%
%          Das Ergebnis der Auswertungen erscheint im Command Window. Es
%          wird dabei zeilenweise ausgewertet. Wird die Programmzeile mit
%          einem Semikolon abgeschlossen, so wird die Ausgabe unterdrückt.
%
...
%  All rights reserved.
%
%  See also stud_matlab_intro_part2, stud_matlab_intro_part3, stud_mittelwert.

% Beim Start eines neuen Programmes sollten zunächst einmal alle alten
% Variablen gelöscht und alle offenen Fenster geschlossen werden.

clear variables     % Es werden alle Variablen gelöscht
...
variable_6 = 1j    % Komplexe Einheit. Die 1 vor dem j erhöht die Robustheit
...
if var > 1
    {
        sprintf('Die Variable ist größer als 2') %Ausgabe im Command Window
        % Achten Sie auf die Rueckgabe im Command Window und vergleichen Sie
        % dies mit dem, wenn sie die Klammern um die Statements in den
        % Verzweigungen entfernen
    }
else
    {
...
for i=1:2:10
    disp(strcat('Der aktuelle Index beträgt: ',num2str(i)))
...
while k<5
    fprintf(strcat('Der aktuelle Index beträgt: ',num2str(k),'\n'))
    % Umwandlung von Zahlen in Strings mit dem Befehl num2str
    pause(1)     % Einführen von Pausen bei der Ausgabe
    k=k+2;
end
...
help stud_mittelwert   % Ausgabe der Hilfe

stud_mittelwert(2,3)   % Ausgabe des Ergebnis
...
save sav_dat vasa           % Abspeichern des Vektors vasa in der
                            % Datei sav_dat.mat

vasa2 = [4 5 6];

save sav_dat -append vasa2  %Anhängen des Vektors vasa2 an die
                            % existierende Datei
...
save sav_dat_asc.txt -ascii vasa   % Abspeichern in einer ascii-Datei
                                   % (z.B. txt)
clear all
