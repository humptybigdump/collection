...
def get_rrc_ir(K, n_up, t_symb, beta):
    ...
    if K % 2 != 0:
        ...
        for i in i_steps:
            if t_steps[i] == 0:
                rrc[i] = 1.0 / np.sqrt(t_symb) * (1.0 - beta +
                                                  4.0 * beta / np.pi)
            elif np.abs(t_steps[i]) == t_symb / 4.0 / beta:
                rrc[i] = beta / np.sqrt(2.0 * t_symb) * (
                    (1 + 2 / np.pi) * np.sin(np.pi / 4.0 / beta) +
                    (1.0 - 2.0 / np.pi) * np.cos(np.pi / 4.0 / beta))
            else:
                rrc[i] = 1.0 / np.sqrt(t_symb) * (
                    np.sin(np.pi * t_steps[i] / t_symb *
                           (1 - beta)) + 4.0 * beta * t_steps[i] / t_symb *
                    np.cos(np.pi * t_steps[i] / t_symb * (1 + beta))) / (
                        np.pi * t_steps[i] / t_symb *
                        (1.0 - (4.0 * beta * t_steps[i] / t_symb)**2.0))
...
def get_rc_ir(K, n_up, t_symb, beta):
    ...
    for i in i_steps:
        if t_steps[i] == 0:
            ...
            rc[i] = 2.0 * fN * np.sin(
                wN * t_steps[i]) / wN / t_steps[i] * np.cos(
                    beta * wN *
                    t_steps[i]) / (1.0 - (4.0 * beta * fN * t_steps[i])**2)
