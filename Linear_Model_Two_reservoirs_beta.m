...
tt_soil = 8;                       % transit time soil to GW (hours)
tt_gw = 120;                         % transit time GW to river (hours)
ET = 1 /24;                         % evapotranspiration (mm/dt) (constant)
...
for jj = 2:nsteps 
    ...
    %%beta store
    q_dir(jj)=Colpach.precip(jj)*dt*(Soil(jj-1)/S_max)^b_eta;
      
    % storage change
    Soil(jj)= Soil(jj-1) - q_soil(jj-1) - ET ...
                + (Colpach.precip(jj)-q_dir(jj)) ;
