...
RunMccTest[Mat_,ICond_,Loading_]:=Module[
  {state,\[Nu],\[Kappa]B,\[Lambda]B,\[Phi],MC,ME,M,P,Q,Pc,\[Epsilon]P,\[Epsilon]Q,epor,itime,elastStiff,unknowns,solution,d\[Epsilon]Trial,\
   dTTrial,PTrial,QTrial,yield,loadingQ,\[Lambda],P1,Q1,Pc1,eq1a,eq1b,eq2a,eq2b,eq3,eq4,unknowns1,approxTF,approx,approxi,unknowns1approxi,\
   solution1,dT2,d\[Epsilon]2,\[Lambda]2, d\[Epsilon],dT},

    {ntime, d\[Epsilon], dT}   = { Loading[[1]], N[ Loading[[2]] ], N[Loading[[3]] ] };

   state=Table[{0,0,0,0,0,0,0,0},{i,1,ntime+1}];
   {\[Nu],\[Kappa]B,\[Lambda]B,\[Phi]}=N[ Mat ];
   \[Kappa]B/=Sqrt[3];\[Lambda]B/=Sqrt[3];MC=(Sqrt[2] (6 Sin[\[Phi]]))/(3 (3-Sin[\[Phi]]));ME=(Sqrt[2] (6  Sin[\[Phi]]))/(3 (3+Sin[\[Phi]]));
   ...
   For[itime=2,itime<=ntime+1,
       ...
       {PTrial,QTrial}={P,Q}+dTTrial;yield=PTrial^2+(QTrial/M)^2-PTrial Pc;
       If[yield>-0.00001 * PTrial^2,loadingQ=True,loadingQ=False];
       (* ELSTIC CASE :::::::::::: *)
       If[!loadingQ,{\[Epsilon]P,\[Epsilon]Q}+=d\[Epsilon]Trial;  epor+=-(1+epor) d\[Epsilon]Trial[[1]] Sqrt[3];   {P,Q}+=dTTrial;  Goto[nextInc]];
       ...
       eq2b=Q1==Q+dT[[2]];            (* updated Q *)
        (* eq4=Pc1==(1+(\[Lambda] (2 P1-Pc1))/(\[Lambda]B-\[Kappa]B)) Pc;  obsolete updated preconsolidation pressure Pc *)
       eq4=Pc1== Pc * Exp[  \[Lambda] (2 P1-Pc1) / (\[Lambda]B-\[Kappa]B)    ]   ;
       ...
       (*-------------------------------------------------------------------------------*)
       solution1=FindRoot[{eq1a,eq1b,eq2a,eq2b,eq3,eq4},unknowns1approxi,AccuracyGoal->5]; (* Essential solution of the invremental problem via RMI *)
       (*-------------------------------------------------------------------------------*)
       If[ solution1[[3,2]]<0,Print[Style["ERROR: \[Lambda]<0, usually when stress increments surpass the (over)critical yield surface ",Red,16, Bold] ]; Abort[] ]; (* Extra control to avoid negative lambda *)
...
epsPepsQPlot[Paths_, Mat_] := Module[{nPaths=1,epsPepsQ, gepsPepsQ,  outputPlot, currentPath , iPath },
 ...
 inputCheck[Mat_, Icond_, Loading_]:= Module[ {P, Q,  \[Nu], \[Kappa]B, \[Lambda]B, \[Phi], Pc, PcPlus, OCR, \[Epsilon]P, \[Epsilon]Q, epor, ntime, dT, deP, deQ, dP, dQ , MC,ME,M,  \[Epsilon]Ppl,\[Epsilon]Qpl},
...
EndPackage[ ]
$Context = "PQ`MCC`"   ;
Print[ "The Modified Cam Clay (MCC) Model for isomorphic P-Q space; by A. Niemunis 2007 \
\n  You are in 2016 in the context PQ`MCC` which provides functions: \
