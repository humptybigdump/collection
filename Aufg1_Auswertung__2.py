...
import Aufg1_Modell as ModellPy
import Aufg1_ODB as DatenbankPy
...
for i in elementGroesse:
    ModellPy.modellErstellung(i, varLoop) 
    varLoop +=1

# Speichern der Verschiebungen im Array abaqusLoesung
for i in range(0,len(elementGroesse)):
    seqVerlauf = DatenbankPy.getVerlauf(i)
    abaqusLoesung += [seqVerlauf]

# Berechnung der anayltischen Loesung
varLoop = 0.0
while varLoop <= 1000.0:
    analytischeLoesung += [(varLoop/1000.0, funcAnalytischeLoesung(varLoop) )]
    varLoop += 20.0
