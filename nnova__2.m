...
(*    Mathematica  rules to simplify tensorial expressions (including Frechet derivatives fD[]) written by A. Niemunis ...2022
...
The predefined Kronecker symbol is \[Delta][i,j] and  delta[i,j].
...
A tensor must  be typewritten as an indexed variable e.g. x[i,j] or as tr[x] or norm2[x]  and indices should be symbolic (not x[1,2]).
 ...
 RECENT CHANGES:
 20.02.2022 addo  
 27.02.2022 fD[Log[xx] , T[]] and Log[a T[i,j]]  added 
...
\[Alpha]::usage = "\[Alpha][i,j] direction cosine matrix predefined and protected ";
...
addo ::usage = "Syntax: addo[\[Alpha]]  \n adds the symbol \[Alpha] to the list 'allorthogonals' with  direction cosine matrices   
                 (for rotations but not for reflections)  " ;             
...
all4tensors::usage = "all4tensors returns the list of all 4th rank tensors";
 ...
 assumeNotUnitary::usage = "assumeNotUnitary[T] \n  deactivatates  the rule norm2[T] := 1  if set " ;
...
different4Q::usage = "different4Q[i,j,k,l] this query returns True because all arguments are different. different4Q[k,j,k,l] returns False" ;
...
\[Delta]4s::usage=" \[Delta]4a beautifier's  abbreviation for symm. identity  tensor. It will not be recognized by other rules so use delta[i,j,k,l ] in the input line ";
...
T::usage=" T is an input of beautifierT and by this usage it is rendered   global ";
e::usage=" e is an input of beautifierE and by this usage it is rendered   global ";
...
delta::usage = " delta[i,j]  identity tensor  ";
deltaI::usage = " deltaI[i,j,k,l]  identity tensor with symmetrization   ";
deltaJ::usage = " deltaJ[i,j,k,l]  identity tensor without symmetrization  ";
...
ricci::usage = "ricci[i,j,k]  denotes the permutation symbol. Simplifying rules are provided. 
           Arguments are always converted to alphabetic order (possibly changing sign)    
           and if  possible ricci is replaced by Kroneckers \[Delta] s    " ;
...
scalarQ::usage=" scalarQ[expr__ ] returns True if expression without any indexed tensors ";
...
len2::usage=" len2[v] = squared length of a vector ";
...
(***** primitives *******)
alltensors = {\[Delta]}  ;
deltaJ[i_,j_,k_,l_] :=    \[Delta][i,k] \[Delta][j,l] ; 
deltaI[i_,j_,k_,l_] :=  (\[Delta][i,l] \[Delta][j,k]+\[Delta][i,k] \[Delta][j,l])/2; 
...
withANitQ[expr__] :=
      Module[ {i,answers={ }},   (* checks if an expression contains at least one of alltensors with indices *)
           answers={ FreeQ[ expr, mult[_,_,_,_] ]  , FreeQ[ expr, mult[_,_,_,_,_] ],  FreeQ[ expr, pow[_,_,_,_]],   FreeQ[ expr,fD[_,_] ]  } ;
           Do[ answers= Append[answers,  FreeQ[ expr, alltensors[[i]][_,_]  ]  ];,
              {i,Length[alltensors]}] ;
            MemberQ[answers,False]
            ] ;
            
scalarQ[expr__] := !withANitQ[expr ] ; 

                        
(* repaired 21.01.2013 and completed 31.01.2022 *)
withTHEitQ[expr__,T_] :=   !FreeQ[ expr, T[_,_]  ]         ||  !FreeQ[ expr, pow[T,_,_,_]]  || 
                            !FreeQ[ expr, mult[T,_,_,_]  ] || !FreeQ[ expr, mult[_,T,_,_]]   ||
                            !FreeQ[ expr, mult[T,_,_,_,_]  ] || !FreeQ[ expr, mult[_,T,_,_,_]] || !FreeQ[ expr, mult[_,_,T,_,_]] || 
                            !FreeQ[ expr, pow[T,_,_,_]]  ;   (* checks if an expr  contains T with indices *)
...
addst[x_] := Module[{n},   (*add a single (!) tensor*)
                     If[!MemberQ[alltensors,x] && !NumberQ[x],
                        ...
                        Print[ "You have ", Length[alltensors]  , " symmetric tensor(s): ",  alltensors, 
                               " \n Mma multiplies factors alphabetically:  B[i,k] * A[k,j] -> mult[A,B,j,i ] "
                                ];
                     ];
...
addo[\[Alpha]_] := Module[{n},   (*add a single (!) tensor*)
                     If[!MemberQ[allorthogonals,x] && !NumberQ[x],
                        {allorthogonals=Append[allorthogonals, \[Alpha]];   
                        Unprotect[Plus,Times];
                      \[Alpha][a_?iQ,i_?iQ] \[Alpha][a_?iQ,j_?iQ]   := \[Delta][i,j];
					 \[Alpha][i_?iQ,a_?iQ] \[Alpha][j_?iQ,a_?iQ]   := \[Delta][i,j];
					 \[Alpha][a_?iQ,i_?iQ] \[Delta][a_?iQ,j_?iQ]   :=\[Alpha][j,i];
					 \[Alpha][i_?iQ,a_?iQ] \[Delta][j_?iQ,a_?iQ]   := \[Alpha][i,j];
					 \[Alpha][a_?iQ,i_?iQ] \[Alpha][b_?iQ,j_?iQ] \[Alpha][c_?iQ,k_?iQ]  ricci[i_?iQ,j_?iQ,k_?iQ]   := ricci[a,b,c];
					 \[Alpha][i_?iQ,a_?iQ] \[Alpha][j_?iQ,b_?iQ] \[Alpha][k_?iQ,c_?iQ]  ricci[i_?iQ,j_?iQ,k_?iQ]   := ricci[a,b,c];
					  \[Alpha][a_?iQ,b_?iQ] (c_+d_):=\[Alpha][a,b] c+\[Alpha][a,b] d;
					 (c_+d_) \[Alpha][a_?iQ,b_?iQ]:=\[Alpha][a,b] c+\[Alpha][a,b] d; 
                        Protect[Plus,Times, \[Alpha]];
                           }   ]; (*if*)
                        Print[ "You have ", Length[allorthogonals]  , " orthogonal(s): ",  allorthogonals, " for rotation but not for reflection " ];
                     ];                     
...
ricci /: ricci[i_?iQ, j_?iQ, k_?iQ] * z_.* v_?vQ[a_?iQ]*v_[b_?iQ] := 0 /; Length[Union[{i,j,k,a,b}]] == 3 ;     (* 22.04.2022 *)  
...
Unprotect[Plus,Times,Log] ;
 Log[ A_?scalarQ* T_?tQ[i_?iQ,j_?iQ]] := Log[A] \[Delta][i,j]  + Log[T[i,j]] /; !MatchQ[i,j]  ;    (* Jan. 2022*)
 Log[ T_?tQ[i_?iQ,j_?iQ] * Z_?scalarQ] := Log[Z] \[Delta][i,j]  + Log[T[i,j]] /; !MatchQ[i,j]  ;    (* Jan. 2022*)
...
pow/:  pow[T_?tQ, n_, i_ , j_ ] * z_.* \[Delta][i_, k_]  :=   pow[T, n, k , j ]   ;       (*27.01.2022*)
pow/:  pow[T_?tQ, n_, i_ , j_ ] * z_.* \[Delta][j_, k_]  :=    pow[T, n, i, k ]  ;      (*27.01.2022*)
...
pow/:  pow[T_?tQ, n_, i_ , j_ ]  -  pow[T_, n_, j_ , i_ ] := 0       ;       (*  symmetry *)
...
mult/: mult[T_?stQ,T_,i_,j_] *  T_[ j_ , k_ ]        :=     pow[T, 3, i  , k ]  /; !MatchQ[i,k]  ;    (* 27.01.2022*)
mult/: mult[T_?stQ,T_,i_,j_] *  T_[ a_ , j_ ]        :=     pow[T, 3, a , i ]  /; !MatchQ[a,i]  ;    (* 27.01.2022*)
mult/: mult[T_?stQ,T_,i_,j_] *  T_[ i_ , k_ ]        :=     pow[T, 3, j  , k ]  /; !MatchQ[j,k]  ;    (* 27.01.2022*)
mult/: mult[T_?stQ,T_,i_,j_] *  T_[ a_ , i_ ]        :=     pow[T, 3, j  , a ]  /; !MatchQ[j,a]  ;    (* 27.01.2022*)
...
(* mult/:   HoldPattern[mult[x_,x_,j_?iQ,k_?iQ]^2] :=  4 * qubic[x] * tr[x] /3 + tr[x]^4 / 6  - (tr[x])^2 * norm2[x]  + (norm2[x])^2 / 2 ;    chyba zle bo kwdrat to mnozenie z jedna kropka *)
...
(* triple mult rules  30.01.2022 *)
mult/:   mult[A_?tQ , B_?tQ, i_?iQ, j_?iQ] * C_?tQ[ i_?iQ, k_?iQ ] :=  mult[C,A,B , k, j]  /; !MatchQ[ j,k] ;    
mult/:    mult[A_?tQ , B_?tQ, i_?iQ, j_?iQ] * C_?tQ[ j_?iQ, k_?iQ ] :=  mult[A,B ,C ,i, k]  /; !MatchQ[ i,k] ;   
mult/:   mult[A_?tQ , B_?tQ, i_?iQ, j_?iQ] * C_?tQ[  k_?iQ ,i_?iQ] :=  mult[C,A,B , k, j]  /; !MatchQ[ j,k] ;    
mult/:    mult[A_?tQ , B_?tQ, i_?iQ, j_?iQ] * C_?tQ[  k_?iQ ,j_?iQ ] :=  mult[A,B ,C ,i, k]  /; !MatchQ[ i,k] ;   

mult/:   C_?tQ[ i_?iQ, k_?iQ ] * mult[A_?tQ , B_?tQ, i_?iQ, j_?iQ] :=  mult[C,A,B , k, j]  /; !MatchQ[ j,k] ;    
mult/:   C_?tQ[ j_?iQ, k_?iQ ]*   mult[A_?tQ , B_?tQ, i_?iQ, j_?iQ]   :=  mult[A,B ,C ,i, k]  /; !MatchQ[ i,k] ;  
mult/:   C_?tQ[ k_?iQ , i_?iQ ] * mult[A_?tQ , B_?tQ, i_?iQ, j_?iQ] :=  mult[C,A,B , k, j]  /; !MatchQ[ j,k] ;    
mult/:   C_?tQ[  k_?iQ,j_?iQ  ]*   mult[A_?tQ , B_?tQ, i_?iQ, j_?iQ]   :=  mult[A,B ,C ,i, k]  /; !MatchQ[ i,k] ;   

mult/:   mult[A_?tQ , B_?tQ, \[Delta],  i_?iQ, j_?iQ] * C_?tQ[ j_?iQ, k_?iQ ] :=  mult[A,B ,C,  i,k]  /; !MatchQ[ i,k] ;    
mult/:   mult[A_?tQ , B_?tQ, \[Delta],  i_?iQ, j_?iQ] * C_?tQ[ k_?iQ, j_?iQ ] :=  mult[A,B ,C,  i,k]  /; !MatchQ[ i,k] ;    

mult/:    mult[ \[Delta] ,A_?tQ, B_?tQ, i_?iQ, j_?iQ] * C_?tQ[ i_?iQ, k_?iQ ] :=  mult[C, A,B ,  k,j ]  /; !MatchQ[ j,k] ;   
mult/:    mult[ \[Delta] ,A_?tQ, B_?tQ, i_?iQ, j_?iQ] * C_?tQ[ k_?iQ, i_?iQ ] :=  mult[C, A,B ,  k,j ]  /; !MatchQ[ j,k] ;   

mult/:    mult[  A_?tQ, B_?tQ, C_?tQ,  i_?iQ, j_?iQ] * \[Delta][ i_?iQ, k_?iQ ] :=  mult[  A,B ,C , k,j ]  /; !MatchQ[ j,k] ;   
mult/:    mult[  A_?tQ, B_?tQ, C_?tQ, i_?iQ, j_?iQ] * \[Delta][ k_?iQ, i_?iQ ] :=  mult[ A,B ,C , k,j ]  /; !MatchQ[ j,k] ;   
mult/:   \[Delta][ i_?iQ, k_?iQ ] *  mult[  A_?tQ, B_?tQ, C_?tQ,  i_?iQ, j_?iQ]  :=  mult[  A,B ,C  k,j ]  /; !MatchQ[ j,k] ;   
mult/:  \[Delta][ k_?iQ, i_?iQ ] *   mult[  A_?tQ, B_?tQ, C_?tQ, i_?iQ, j_?iQ]  :=  mult[ A,B ,C , k,j ]  /; !MatchQ[ j,k] ;   

mult/:    mult[  A_?tQ, B_?tQ, C_?tQ,  i_?iQ, j_?iQ] * \[Delta][ j_?iQ, k_?iQ ] :=  mult[  A,B ,C , i,k ]  /; !MatchQ[ j,k] ;   
mult/:    mult[  A_?tQ, B_?tQ, C_?tQ, i_?iQ, j_?iQ] * \[Delta][ k_?iQ, j_?iQ ] :=  mult[ A,B ,C , i,k ]  /; !MatchQ[ j,k] ;   
mult/:   \[Delta][ j_?iQ, k_?iQ ] *  mult[  A_?tQ, B_?tQ, C_?tQ,  i_?iQ, j_?iQ]  :=  mult[  A,B ,C,  i,k ]  /; !MatchQ[ j,k] ;   
mult/:  \[Delta][ k_?iQ, j_?iQ ] *   mult[  A_?tQ, B_?tQ, C_?tQ, i_?iQ, j_?iQ]  :=  mult[ A,B ,C , i,k ]  /; !MatchQ[ j,k] ;   
mult/:   mult[  A_?tQ, B_?tQ, A_?tQ, i_?iQ, j_?iQ]  +  mult[  A_ , B_ , A_ ,  j_ ,  i_  ] :=   2 mult[  A, B, A, i, j]   /; !MatchQ[ i,j];  
mult/:    mult[  A_?tQ, B_?tQ, B_?tQ, i_?iQ, j_?iQ]  +    mult[  B_ , B_ , A_ ,  j_ ,  i_  ] :=   2 mult[  A, B, B, i, j]  /; !MatchQ[ i,j] ; 
mult/:   -mult[  A_?tQ, B_?tQ, A_?tQ, i_?iQ, j_?iQ]  +  mult[  A_ , B_ , A_ ,  j_ ,  i_  ] :=  0  /; !MatchQ[ i,j];  
mult/:   - mult[  A_?tQ, B_?tQ, B_?tQ, i_?iQ, j_?iQ]  +    mult[  B_ , B_ , A_ ,  j_ ,  i_  ] :=   0 /; !MatchQ[ i,j] ; 
mult/:    mult[  A_?tQ, B_?tQ, A_?tQ, i_?iQ, j_?iQ]  -  mult[  A_ , B_ , A_ ,  j_ ,  i_  ] :=  0  /; !MatchQ[ i,j];    (* must  be privitive otherwise 'too deep pattern' *)
mult/:     mult[  A_?tQ, B_?tQ, B_?tQ, i_?iQ, j_?iQ]  -    mult[  B_ , B_ , A_ ,  j_ ,  i_  ] :=   0 /; !MatchQ[ i,j] ; 


mult/:   mult[  A_?tQ, A_ , A_ , i_?iQ, j_?iQ ]  := pow[A ,3, i,j] /; !MatchQ[ i,j] ;    
(* end of triple mult rules  30.01.2022 *)
...
Unprotect[Plus,Minus,Divide,Power,Times] ;
  ...
  x_?tQ[i_?iQ,k_?iQ] * z_. * y_?tQ[k_?iQ,j_?iQ]  :=   mult[x,y,i,j]* z    /; !MatchQ[i,j] ;    (* nonsymmetric output *)  
  x_?tQ[k_?iQ,i_?iQ ] * z_. * y_?tQ[j_?iQ, k_?iQ]  := mult[x,y,i,j] z        /; !MatchQ[i,j] ;       
  x_?tQ[i_?iQ,k_?iQ] * z_. * y_?tQ[j_?iQ,k_?iQ]  := mult[x,y,i,j] z        /; !MatchQ[i,j] ;    
  x_?tQ[k_?iQ,i_?iQ] * z_. * y_?tQ[j_?iQ,k_?iQ]  := mult[x,y,i,j] z        /; !MatchQ[i,j] ;    
  x_?tQ[k_?iQ,i_?iQ] * z_. * y_?tQ[k_?iQ,j_?iQ]  := mult[x,y,i,j] z        /; !MatchQ[i,j] ;    
 ...
 (* x_?tQ[a_?iQ,b_?iQ]^(n_?OddQ) :=  x[a,b] norm2[x]^((n-1)/2)  /;  !MatchQ[a,b] && n>1 ;  chyba zle  bo kwadrat to mnozenie z jedna kropka *)
  ...
  HoldPattern[  a_. mult[x_,y_,i_?iQ,j_?iQ] +  b_. mult[y_,x_,j_?iQ,i_?iQ]  ] /; stQ[x] && stQ[y]  :=   (a+b) * mult[x,y,i,j]  ;  (* new 21.9.06 changed 24.10.06 *)
 (*
   (a_.*x_?stQ[i_,j_] + b_.*y_?stQ[i_,j_])^2 := Simplify[a^2 norm2[x] +     2 a b scalar[x,y]   + b^2 norm2[y]] ;   
   *)       (* ambiguous expression interpreted as scalar, used to work before 2022  *)
Protect[Plus,Minus,Divide,Power,Times] ;
...
fD/:  fD[a_ *  b_ , c_?tQ[i_?iQ,j_?iQ]]  := Simplify[ fD[a,c[i,j]] * b  +  a  * fD[b, c[i,j]]  ] ;
...
fD/:  fD[Logt[a_],c_?tQ[i_?iQ,j_?iQ] ] := fD[a,c[i,j]] / ( a )  /; !FreeQ[a, c]  && !withTHEitQ[a,c] ;   (* c jest w  a  ale  bez indexow  *)
...
*) 
(* the last three lines work without HoldPattern   28.01.2022 *) 
fD /:  fD[ mult[y_, x_, i_?iQ, j_?iQ], x_[p_?iQ, q_?iQ]]  /; ! MatchQ[x, y] :=    ( y[i, p] \[Delta][j, q] +  y[i, q] \[Delta][j, p] )/2;   
fD /:  fD[ mult[x_, y_, i_?iQ, j_?iQ], x_[p_?iQ, q_?iQ]]  /; ! MatchQ[x, y] :=    ( y[q, j] \[Delta][i, p] +  y[p, j] \[Delta][i, q] )/2  ;   

fD/:   fD[ mult[T_, T_, i_?iQ, j_?iQ], T_[a_?iQ,b_?iQ] ] := (T[b,j] \[Delta][a,i]+T[b,i] \[Delta][a,j]+T[a,j] \[Delta][b,i]+T[a,i] \[Delta][b,j])/2;      (* January 2022 *)

fD/:   fD[ pow[T_?tQ, 3, i_?iQ, j_?iQ], T_[a_?iQ,b_?iQ] ] := (T[a,j] T[b,i]+T[a,i] T[b,j]+mult[T,T,\[Delta],j,b] \[Delta][a,i]+mult[T,T,\[Delta],i,b] \[Delta][a,j]+
                                                           mult[T,T,\[Delta],j,a] \[Delta][b,i]+mult[T,T,\[Delta],i,a] \[Delta][b,j] )/2 ;  (* 30.01.2022 *) 
                                                                     
 fD/:  fD[pow[T_?tQ, 4, i_, j_  ], T_[a_, b_]]   /;   different4Q[i,j,a,b]   :=  
                                                     (  mult[T,T,\[Delta],j,b] T[a,i]+mult[T,T,\[Delta],i,b] T[a,j]+mult[T,T,\[Delta],j,a] T[b,i]+
                                                       mult[T,T,\[Delta],i,a] T[b,j]+pow[T,3,j,b] \[Delta][a,i]+pow[T,3,i,b] \[Delta][a,j]+
                                                       pow[T,3,j,a] \[Delta][b,i]+pow[T,3,i,a] \[Delta][b,j] )/2 ;            (* 30.01.2022 *) 

fD/:  fD[Log[y_],x_?tQ[i_?iQ,j_?iQ]] := fD[y,x[i,j]]/y   /;  !FreeQ[y,x]  && !withTHEitQ[y,x]  ;  (* w a jest c ale  bez indexow  *)
fD/:  fD[Log[T_?tQ[i_?iQ,j_?iQ] ], T_?tQ[a_?iQ,b_?iQ] ] := If[  FreeQ[z, x],   Message[ nnova::"fDLog", T  ] ; ( pow[T[i,a], -1] \[Delta][j,b] + pow[T[i,b], -1] \[Delta][j,a]   )/2   ];    
 (*  31.01.2022  the above derivative is  experimental  works for disturbances coaxial with T only  *)

fD/:  fD[Exp[y_],x_?tQ[i_?iQ,j_?iQ]] :=  Exp[y]*fD[y,x[i,j]]   /;  !FreeQ[y,x]  && !withTHEitQ[y,x]  ;
fD/:  fD[scalar[x_,y_,z_],a_?tQ[i_?iQ,j_?iQ]] := If[SameQ[x, a] , (mult[y,z,j,i]+ mult[y,z,i,j])/2,0]+ If[SameQ[y,a], (mult[x,z,i,j] + mult[x,z,j,i])/2,0] + If[SameQ[z,a], (mult[x,y,i,j] + mult[x,y,j,i])/2,0];

fD /: fD[ m_. *  A_?t4Q[i_?iQ, j_, k_, l_]  d_[k_, l_]  , d_[a_, b_]   ] :=   m  A[i, j, a, b]  ;
fD /: fD[ m_. *  A_?t4Q[i_?iQ, j_, k_, l_]  d_[i_, j_]   , d_[a_, b_]   ] :=   m  A[a, b, k, l]  ;
fD /: fD[ m_. *  A_?t4Q[i_?iQ, j_, k_, l_]  d_[k_, l_]   d_[i_, j_] ,    d_[a_, b_]   ] :=   m  A[i, j, a, b]  d[i, j] +  m  A[a, b, k, l]  d[k, l]   ;
fD /: fD[ m_. *  A_?t4Q[i_?iQ, j_, k_, l_]  d_[i_, j_]  d_[k_, l_],    d_[a_, b_]   ] :=     m  A[i, j, a, b]  d[i, j] +  m  A[a, b, k, l]  d[k, l]   ;
...
fD/:  fD[pow[x_?tQ, -1, i_, i_  ], x_[r_, s_]] :=  -pow[x,-2, r, s]  /; !MatchQ[r,s];
...
fD/:  fD[pow[T_?tQ,   1 , i_, j_ ] , T_[s_ ,t_ ]   ] :=   deltaI[i,j,s,t]  ;
...
zoutSym/: HoldPattern[zoutSym[ a_?tQ[i_ ,j_ ]  , mult[ b_, c_,   k_ , l_ ]    ] ]:=   ( a[i,k] mult[b,c, j,l] + a[i,l] mult[b,c, j,k]  ) /2;
zoutSym/: HoldPattern[zoutSym[mult[a_, b_, i_,j_]  , c_?tQ [k_ , l_ ]    ]] :=   (  mult[a, b, i, k] c[j,l]  +  mult[a, b, i, l] c[j,k]   )/2;
...
subscriptForm={A_?tQ [i_?NumberQ ,j_?NumberQ]:>Subscript[A,  i*10+ j  ] ,
                               ...
                               A_?tQ[i_ ,j_ ]   :>Subscript[A, HoldForm[ i  j]]  /;   \[Not]NumberQ[Evaluate[ i*j]]   ,
                                A_?vQ[i_  ]   :>Subscript[A, HoldForm[ i ]]  /;   \[Not]NumberQ[Evaluate[ i]]   ,
                                A_?gvQ[i_ ,j_ ]   :>Subscript[A, HoldForm[ i ; j]]  /;   \[Not]NumberQ[Evaluate[ i*j]]   ,
                                A_?gvQ[i_ ,j_ ,k_]   :>Subscript[A, HoldForm[ i ; j k]]  /;   \[Not]NumberQ[Evaluate[ i*j*k]]   , 
...
(* Messages *)
 ...
 nnova::"Globals" = "Syntax warning: possibly  unintended usage of globals.  This function is using global variable(s):  `1`   `2` ." ;
 nnova::"fDLog" = "warning: experimental derivative assuming that the differential  d`1` is coaxial with `1`" ;

 End[ ] ;  (* private *)

Protect[
addst, addo,addv, addst4, assumeDeviatoric, assumeMajorSymmetric, assumeMinorSymmetric, assumeNotDeviatoric, assumeNotUnitary, assumeUnitary,
...
SetOptions[$FrontEndSession,{"PageWidth"->1000,"ExportTypesetOptions"->{"PageWidth"->1000}}];
  Print[ "You are in the context Tensor`nnova`;  ver. April 2022.  It simplifies index-expressions with Cartesian tensors.\
     \n DECLARATIONS: addst addo addv, addgv, alltensors allorthogonals noLongerTensor noLongerOrthogonal   \
     \n                 useInvariants    useRoscoeLode     assumeDeviatoric  assumeUnitary   assumeMajorSymmetric   assumeMinorSymmetric \
     \n                  assumeNotDeviatoric   assumeNotUnitary \
     \n PREDEFINED OBJECTS:  delta=\[Delta]  \[Alpha]  T  ricci deltaJ deltaI  \[Delta]Dev iC iE      \
     \n QUERIES :  iQ,  tQ, stQ,  oQ  vQ  gvQ different3Q qubiclikeQ,  withtQ,  withANitQ  different4Q , withTHEitQ  \
     \n FUNCTIONS : fD,   deviator  tr mult   norm2  normalized hated   \
     \n               qubic   scalar     transfer99, transfer99ijkl,   transfer33   pow  \
     \n DELAYED RULES:   //. beautifierT, //. beautifierE  (with internal abbreviations Td, ed Dev4 \[Delta]4s \[Delta]4)\
     \n                   //. numerical (to be used after transfer99) \
     \n                   //. subscriptForm  \
     \n EXPONENTS (EVEN != ODD): x[i,j]^(2n) = (x[i,j] x[i,j])^n = (norm2[x])^n != pow[x,2n,i,j], but x[i,j]^(2n+1)= pow[x,2n +1 ,i,j]\
     \n SPECIAL:   zoutSym  multiplication from Itskov book \
      " ];
