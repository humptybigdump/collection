...
g_u_sobel = conv2(I_d, sobel_fm_u, 'same');
g_v_sobel = conv2(I_d, sobel_fm_v, 'same');
g_u_sobel_alternative = imfilter(I_d, sobel_fm_u, 'conv');
g_v_sobel_alternative = imfilter(I_d, sobel_fm_v, 'conv');
...
figure;
smplot(2,1,1);
imshow(g_sobel_angle, [-pi, pi], 'Colormap', hsv);
colorbar;
smplot(2,1,2);
