...
while Erfolge<stichpr_n,
   ...
   if rand(1,1)<f(x,0,0)/(1/(2*pi))/k,  % vergleicht Dummy mit p=h(x)/(p*g(x))
       Erfolge=Erfolge+1;
       X_stich(Erfolge)=x;
   end
end
...
tic();
X_stich2=rand(stichpr_n*2.1,1)*2*pi;    % 2.1 kommt aus Verhältnis Erfolge/Versuche
dummy=rand(stichpr_n*2.1,1);
X_stich2(f(X_stich2,0,0)/(1/(2*pi))/k<dummy)=[];  
...
X_stich3(f(X_stich3(:,1),X_stich3(:,2),0)/(1/(4*pi^2))/k<dummy,:)=[];
