...
Gamma = 0.1;                    % diffusive coefficient

% numerical parameters
delta_t = 0.003;                        % time step
t_max = 2.0;                            % maximum simulation time
n_nodes = 31;                           % number of nodes
iterations_max = ceil(t_max/delta_t);   % maximum number of iterations
delta_x = L/(n_nodes-1);                % cell size

% output parameters
f_output = 5;                   % frequency of the graphical output

%% initialization of necessary vectors and values
phi_bds = zeros(n_nodes,1);
phi_cds = zeros(n_nodes,1);
dphi_dt_bds = zeros(n_nodes,1);
dphi_dt_cds = zeros(n_nodes,1);

t = 0;  % time

% Spatial position of the grid nodes
x_disc = linspace(0, L, n_nodes);
...
p_FDM = plot(x_disc,phi_bds,'-ro',x_disc,phi_cds,'-kx');
p_FDM(1).XDataSource = 'x_disc';
p_FDM(1).YDataSource = 'phi_bds';
p_FDM(2).XDataSource = 'x_disc';
p_FDM(2).YDataSource = 'phi_cds';
...
%% time loop
for iteration = 1:iterations_max
    
    for node=2:n_nodes-1
        ...
        % Euler scheme
        phi_bds(node) = 'fill in!'; % IMPLEMENT THE INTEGRATION HERE USING phi_bds!
        phi_cds(node) = 'fill in!'; % IMPLEMENT THE INTEGRATION HERE USING phi_cds!
    end
