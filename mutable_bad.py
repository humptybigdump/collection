def create_dict_and_append_key_value_pair(key, value, newdict={}):
    """
    Add a key-value pair (`key`, `value`) to the dictionary `newdict`. 
    If no dictionary is provided, it modifies the default dictionary.

    Parameters:
    key (any): The key to add to the dictionary.
    value (any): The value to associate with the key in the dictionary.
    newdict (dict, optional): The dictionary to add the key-value pair to. Defaults to an empty dictionary (`{}`).
