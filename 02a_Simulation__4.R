...
auxFe = swarmPlot(wind.compo.osim.aux, PLOTFUN=myQQfun, j="Fe2O3",
...
vgFe = gstat(id="Fe2O3", formula=Fe2O3~1, locations=~Easting+Northing,
          data = cbind(data.frame(wind.acomp), wind.coords)) %>%
  variogram(cutoff=50)

myVGfun = function(x){
  vg = gstat(id="Fe2O3", formula=Fe2O3~1, locations=~Easting+Northing,
...
# Change the code to check different elements and note which ones are well
