#Financial Econometrics II Problem Set 3

#Problem 5: GARCH and stochastic volatility Estimation
...
est_norm_solnp<-ugarchfit(spec_norm,sim_t4_vec,solver="solnp")
...
est_gosolnp<-ugarchfit(spec_norm,sim_t3_vec,solver="gosolnp",solver.control=
                         list(n.starts=500,n.sim=1000,rseed=1234))
#use hybrid tool that tries all different models


###################################
#Problem 6: Value at Risk Forecasting and Evaluation
...
?BacktestVaR
hist_scores<-BacktestVaR(oos_data,forecasts_var_fast[,1],alpha=0.05)
...
#ideally, close to 1
sum(sim_calc[(window_length+1):len_sim]<forecasts_var_fast[,1])/1
