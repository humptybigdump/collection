...
4) Begin a Mathematica session with:    Needs["Tensor`bnova`"]
  ...
  14.4.2023 added replaceSubexpr by Jon McLoone  
  9.05.2023  plotCircleT works with 3x3 with geotechnical and has xPlotRange option  
 *)
...
aRot::usage = "Syntax:  aRot12[t] aRot13[t] aRot23[t] returns the directional cosine matrix 33 for active plane rotation 
 by angle t [radians]  in the plane x1-x2, x1-x3 or x2-x3, see also pRot"  ;

  aRot12::usage = "Syntax:  a = aRot12[t];  returns the directional cosine matrix 33 for active plane rotation
 by  angle t [radians]  in  x1-x2 plane. 
 The angle t > 0  is positive if the rotation points along +x3 axis by the right-hand rule, that means 
 aRot12[90\[Degree]].{1,0,0} =  {0,1,0}
  See also pRot12"  ;
 
 aRot13::usage = "Syntax:  a= aRot13[t] ; returns the directional cosine matrix 33 for active plane rotation
 by  angle t [radians]  in   x1-x3. 
 The angle t > 0  is positive if the rotation points along -x2  (MINUS) axis by the right-hand rule, that means 
 aRot13[90\[Degree]].{1,0,0} =  {0,0,1}
  See also pRot13"  ;
 
 aRot23::usage = "Syntax: a = aRot23[t];   returns the directional cosine matrix 33 for active plane rotation
 by angle  t [radians]  in   x2-x3. 
 The angle t > 0  is positive if the rotation points along +x1 axis by the right-hand rule, that means 
 aRot23[90\[Degree]].{0,1,0} =  {0,0,1}
  See also pRot23"  ;

pRot::usage = "Syntax:  a = pRot12[t]; returns the directional cosine matrix 33 for passive plane rotation
 by  angle t [radians]  in the plane x1-x2, x1-x3 or x2-x3. 
  See also aRot" ;

pRot12::usage = "Syntax: a = pRot12[t] pRot13[t] pRot23[t] returns the directional cosine matrix 33 for passive plane rotation
 by  angle t [radians]  in  x1-x2. 
 The angle t > 0  is positive if the rotation points along +x3 axis by the right-hand rule , that means 
 pRot12[90\[Degree]].{1,0,0} =  {0,-1,0}
  See also aRot" ;

 pRot13::usage = "Syntax:  a=   pRot13[t];  returns the directional cosine matrix 33 for passive plane rotation
 by  angle t [radians]  in   x1-x3. 
 The angle t > 0  is positive if the rotation points along -x2  (MINUS) axis by the right-hand rule, that means 
  pRot13[90\[Degree]].{1,0,0} =  {0,0,-1}
  See also aRot" ;
 
 pRot23::usage = "Syntax:  a =   pRot23[t];  returns the directional cosine matrix 33 for passive plane rotation
 by  angle t [radians]  in  x2-x3. 
 The angle t > 0  is positive if the rotation points along +x3 axis by the right-hand rule, that means 
 pRot12[90\[Degree]].{0,1,0} =  {0,0,-1}
  See also aRot" ;
 
rotateTensor::usage = "Syntax:  Tp = rotateTensor[T, a];   rotates the tensor T  using the 33 rotation matrix a according to the 
 tensorial transformation rules, for example  T'_ij  = a_ik a_jl  T_kl.       
ARGUMENTS: T  =  a vector or tensor  (type = t3, t33, t333 or t3333) 
                     a  =  an orthogonal 33  rotation matrix =  directional cosine matrix 
OUTPUT    Tp is the object  T with components  after active rotation or expressed 
              in the passively rotated basis.
    Active rotation means that the coordinate system does not change and the object is rotated,   
    for example  rotateTensor[T,  aRot12[30\[Degree]]] tensor T is rotated by 30\[Degree] about the x3 axis 
    Passive rotation means that the coordinate system is rotated and the object does not change,   
    for example  rotateTensor[T,  pRot13[30\[Degree]]] tensor T is rotated by 30\[Degree] about the x2 axis 
    (note that  pRot13[30\[Degree]]  ==   Transposed[ aRot13[30\[Degree]]  ] )
    See also aRot12, pRot12, rotationTensor and rotationTensorInverse ";
...
plotCircleT::usage=" Syntax:   {gA, gB} = plotCircleT[Tb , psi  ] returns two graphics: 
            gA = half space corresponding to the angle psi with the local coordinate system {h,n} of the  cross-section  
                    and the traction vector  acting on the half space   
             gB=  Mohr circle of the stress Tb with the black point = coordinates of the traction vector 
                     in  the local coordinate system {h,n} and the green point of the Pole                 
ARGUMENTS:   Tb = 2x2 stress matrix in x1,x2 plane (tension is positive and x1 is a horizontal axis pointing to the right)  
                  if Tb of dimensions 3x3 is input then the upper left 2x2 part will be extracted and used    
            psi = inclination angle (CCW) of a chosen cross-section  to the horizontal +x1 axis , nb^M = {-Sin[psi], Cos[psi] }.  
                     The stress vector {\[Tau],\[Sigma]} in the local   h-n-system   is  marked on the Mohr-circle by a black point. 
 returns    gA = plot of the physical coordinate system with the traction vector  
            gB = graphics with the Mohr circle its Pol                     
 OPTIONS:  plotRange ->  {{\[Sigma]min , \[Sigma]max}, {\[Tau]min, \[Tau]max}};  unless specified:  m +/- 1.2 Mohr-Radius  is assumed in each direction 
           geotechnical -> False , otherwise  assumes that the input stress is given in geotechnical convention;  
                           The local axes hG and nG of the cross-section given by the angle psi are similar to hM and nM but rotated  by 180\[Degree]
                           The geotechnical unit vector nG points to the inside of the material      
                           Moreover the circle is plotted  for positive  compression \[Sigma]  and the counterclockwise shear \[Tau].
   
           withCoulomb -> {phi[rad],c} unless both parameters zero  adds Coulomb lines (default values {0,0})
           verbose -> False  otherwise produces some output for debugging   
...
Options[plotCircleT] = {withCoulomb -> {0,0}, plotRange -> {0,0}, verbose -> False , geotechnical -> False , xPlotRange -> 0 };  
plotCircleT[Tbi_, a_ , OptionsPattern[] ] :=  Module[
     {Tb, nb, hb, tb, tbnorm, scale, stressPoint, polPoint, m, R,phi,coh,range,sigP,tauP,s, oV,oG,aLabel },
    
   (*   Tb={{T11,T12},{T12,T22}},  -->  x1 and 90\[Degree] CCW x2;  mechanical signs unless geotechnical -> True *)

   {phi, coh} = OptionValue[withCoulomb]; 
   range = OptionValue[plotRange]; 
   oV = OptionValue[verbose]; 
   oG = OptionValue[geotechnical];  
   oX = OptionValue[xPlotRange]; 
   If[ Dimensions[Tbi] == {2,2}, Tb = Tbi,  Tb = Tbi[[1;;2, 1;;2]]  ];    
   {m, R } = getMohrParamsT[ Tb ]  ;  
   If[Dimensions[oX] == {2,2}, xRange = oX] ; 
   If[oX == 0,  x = Norm[Tb, "Frobenius"]; xRange = {{-x,x},{-x,x}}  ];
  
  If[oG,  aLabel = {"\!\(\*SuperscriptBox[\(\[Sigma]\), \(G\)]\) compr > 0", "\!\(\*SuperscriptBox[\(\[Tau]\), \(G\)]\) counterclockwise > 0"}, 
           aLabel = {"\!\(\*SuperscriptBox[\(\[Sigma]\), \(M\)]\) tension > 0", "\!\(\*SuperscriptBox[\(\[Tau]\), \(M\)]\) clockwise > 0"} ];
 
  g1 = Graphics[{ Circle[ {m,0}, R]}, Axes -> True,  AxesLabel -> aLabel,  AxesOrigin -> {0, 0}];  
            
                                                                                   
   If[ oG, hb = -{Cos[a], Sin[a]};   nb = {Sin[a], -Cos[a] },
            hb = {Cos[a], Sin[a]};   nb = {- Sin[a], Cos[a] };
      ];   
   tb =  Tb . nb; tbnorm = Norm[tb]; 
   scale= Which[   tbnorm <= 1 , 0.1,
          1<tbnorm  <= 10, 1 , 
          10 < tbnorm  <= 50, 10,
          50 < tbnorm  <= 100, 50 ,
          50 < tbnorm  <= 100, 50 ,
 ...
 If[oV, Print["scaling unit basis vectors with scale=   ",  scale, " calculated from tbnorm =", tbnorm "  with nb=", nb ]];                
   g01 =  Graphics[{Arrow[  {{0, 0},  scale * hb }],
                Text[ Row[{ Style[ ToString[scale],  Larger],  Style[ "*h",Bold,Larger],  Style[ "[m]",  Larger] } ],  1.1*scale*hb], 
                  Arrow[ {{0, 0},  scale* nb }],
                 Text[  Row[{  Style[ ToString[scale],  Larger],Style[ "*n",Bold,Larger], Style[ " [m]", Larger]}], 1.1*scale*nb],
                 Red, Arrow[  {{0, 0},  tb}],
                  Text [ Row[{Style["t",Bold,Larger], Style[ " [kPa]", Larger] }] , 1.1*tb]
                 },
                 Axes->True, AxesLabel->{"x1","x2"} , PlotRange -> xRange 
                 ];
   ...
   g2 = Graphics[{PointSize[Large], Point[ stressPoint], Green , Point[polPoint] ,
                  Text [Style[ "Pole", Larger], 0.9*polPoint]  } ] ; 
                             
   If[ range == {0,0}, 
       If[oV, Print["using the automatically calculated prescribed PlotRange {-1.2 R, 1.2 R} = ", {-1.2 R, 1.2 R}]];         
        g3 = Plot[ {coh + Tan[phi]*(-s), -coh  - Tan[phi]*(-s) },  {s, m-R, m+R } ,
                      PlotRange -> 1.2*R*{-1,1} ,AxesLabel -> aLabel,  AspectRatio->Automatic    ]     
        ];  
         If[ Dimensions[range]=={2,2}  ,    
              If[oV, Print["using the prescribed plotRange = ",range]];         
              g3 = Plot[{coh + Tan[phi]*(-s), -coh  - Tan[phi]*(-s) } , {s,  range[[1,1]], range[[1,2]]  } , 
                         PlotRange -> range , AxesLabel -> aLabel, AspectRatio->Automatic   ] ;
           ]; 
    ...
    If[ {phi, coh} == {0,0}, g3={}];                
    g4 = Graphics[{ Opacity[0.6, LightGray], Disk[{0,0} ,scale, { -180\[Degree]+a , a  } ] }];                      
   {Show[{g01, g4}],  Show[  {g3, g1, g2 }  ] }
      ];
