% Interactive Matlab GUI to visualize Multiple Access Channel (MAC) Capacity for two transmitters and one receiver

function mac_capacity_visualizer
    % Create the GUI figure
    hFig = figure('Name', 'Multiple Access Channel Capacity', 'NumberTitle', 'off', 'Position', [100, 100, 1000, 600]);
    
    % Create UI controls for transmit power and channel amplification
    uicontrol('Style', 'text', 'Position', [20, 550, 150, 25], 'String', 'Transmit Power P1 (W):');
    hPower1 = uicontrol('Style', 'edit', 'Position', [180, 550, 100, 25], 'String', '1');
    uicontrol('Style', 'text', 'Position', [20, 510, 150, 25], 'String', 'Transmit Power P2 (W):');
    hPower2 = uicontrol('Style', 'edit', 'Position', [180, 510, 100, 25], 'String', '1');
    uicontrol('Style', 'text', 'Position', [20, 470, 150, 25], 'String', 'Path-loss g1:');
    hPathLoss1 = uicontrol('Style', 'edit', 'Position', [180, 470, 100, 25], 'String', '1');
    uicontrol('Style', 'text', 'Position', [20, 430, 150, 25], 'String', 'Path-loss g2:');
    ...
    function updateCapacity(~, ~)
        ...
        switch scheme
            ...
            case 2 % Frequency Sharing
                R1 = a1 * log2(1 + 1/a1 * g1 * P1);
                R2 = a2 * log2(1 + 1/a2 * g2 * P2);
                % Update resource allocation plot
                cla(hAxes1);
                ...
                text(hAxes1, 0.5, a1 / 2, sprintf('1/%.2f * P1: %.2f', a1, P1), 'HorizontalAlignment', 'center', 'Color', 'w');
                text(hAxes1, 0.5, a1 + a2 / 2, sprintf('1/%.2f * P2: %.2f', a2, P2), 'HorizontalAlignment', 'center', 'Color', 'w');
                set(hAxes1, 'XLim', [0, 1], 'YLim', [0, 1]);
                xlabel(hAxes1, 'Time');
                ylabel(hAxes1, 'Frequency');
            ...
            case 3 % Interference Cancellation
                ...
                R1 = a1 * log2(1 + g1*P1) + (1 - a1) * log2(1 + g1*P1 / (1 + g2*P2));
                R2 = (1 - a1) * log2(1 + g2*P2) + a1 * log2(1 + g2*P2 / (1 + g1*P1));
                
                % Ensure rate limits
                R1 = min(R1, log2(1 + g1*P1));
                R2 = min(R2, log2(1 + g2*P2));
                ...
                for x = linspace(0, a1, 20)
                    line(hAxes1, [x, x], [0, 1], 'Color', 'b', 'LineWidth', 1);
                end
                rectangle(hAxes1, 'Position', [a1, 0, 1 - a1, 1], 'FaceColor', 'b', 'EdgeColor', 'k'); % User 2 allocation
                % Add red stripes for User 1 overlay on User 2 allocation
                for x = linspace(a1, 1, 20)
                    line(hAxes1, [x, x], [0, 1], 'Color', 'r', 'LineWidth', 1);
                end
                set(hAxes1, 'XLim', [0, 1], 'YLim', [0, 1]);
                xlabel(hAxes1, 'Time');
                ylabel(hAxes1, 'Frequency');
