...
generate.EMOS.forecast <- function(mu, sigma, obs, opt.pars = NULL){
  # a) Emulate ensemble
  ...
  # pars <- c(0, rep(1/ncol(ensforc), ncol(ensforc)), 1, 1) # Try this!
  ...
  print("Parameters: ")
  print(est$par)
  print(paste0("optimized mean CRPS (training data) = ",est$value/length(obs[ind])))
  if(!is.null(opt.pars)){
  ...
  Ynew      <- dbtst[,"obs"]
  return(cbind(mu.cor, sigma.cor, Ynew))
}
...
for(i in c(1,2,7,8)){
  ...
  # CRPS
  print(paste0("mean CRPS (application data) = ",mean(crps.normal(Y = Y.tst, 
  ...
  plot(x = function(x) get(paste0("Fn",i,".new"))(x) - FnY.tst(x), col = "red", main = paste0("F",i),ylab = paste0("Fn",i,"(x) - FnY(x)"),xlim = c(-7,7),ylim = c(-0.1,0.1))
  abline(h = 0,lty = 2,col = "grey")
  # Probabilistic calibration
...
# For the calculation of the optimal parameters, see solution!

# With the optimal parameters we obtain well calibrated forecasts:
opt.pars = matrix(NA,nrow = 8,ncol = 8)
opt.pars[1,] = c(0,1/17,1/17,1/17,1/17,1/17,680/289,0)
opt.pars[2,] = c(0,1/6,1/6,1/6,1/6,1/6,5/3,0)
opt.pars[7,] = c(-1/15,2/15,2/15,2/15,2/15,2/15,11/6,0)
opt.pars[8,] = c(0,-3/17,-3/17,-3/17,-3/17,-3/17,340/289,0)
par(mfrow = c(2,4))
for(i in c(1,2,7,8)){
  print(paste0("Forecast ",i))
  # Get the postprocessed mu and sigma parameters
  newnormal <- generate.EMOS.forecast(mu = get(paste0("mu",i)), 
                                      sigma = get(paste0("sd",i)), obs = Y,
                                      opt.pars = opt.pars[i,])
  assign(paste0("mu",i,".new"), newnormal[,1])
