...
while (Ea > et) && (it < itmax)
    % use theorem to check whether divergent
    rho = abs(gd(xold));
    if rho > 1
        xr=xold;
        disp('My solution : no iteration performed !!')
        disp('rho > 1, g(x) divergent !!')
        disp('Please provide a convergent iteration formula g(x)!!')
        return
    end
