...
variables <- c("coder", "nameinst", "gender", "aktbez", "scidepend", "discipline", "aktort", "misclassification", "duplicate", "journalist", "passive_actor", "llm_quote", "expbez", "no_insider", "llm_expert")

## Vektor mit Expertenbezeichnungen

expert_terms <- "[Ee]xpert\\w*|[Ss]pezialist\\w*|Fachm[aä]nn\\w*|Fachfrau\\w*|Fachleute\\w*|[Ss]achkundige\\w*|Koryphäe\\w*"
...
save_data <- function(actor, inputs){
  ...
  if(any(as.logical(unlist(inputs[c("misclassification", "duplicate", "journalist", "passive_actor")])))){
    inputs[c("nameinst", "aktbez", "gender", "scidepend", "discipline", "aktort", "llm_quote", "expbez", "no_insider", "llm_expert")] <- NA
  }
  else{
  if(!inputs["aktbez"] %in% c(100, 300)){
    inputs["discipline"] <-  NA
  }
  if(!inputs["aktbez"] == 100){
    inputs["scidepend"] <- NA
  }  
  if(inputs["llm_quote"] == FALSE | is.na(inputs["llm_quote"])){
    inputs["expbez"] <- NA
    inputs["no_insider"] <- NA
  }
  if(inputs["expbez"] == TRUE | is.na(inputs["expbez"])){
    inputs["no_insider"] <- NA
  }
  if(inputs["aktbez"] %in% c(210, 230, 231, 232, 233, 234, 235, 236, 237)){
    inputs["no_insider"] <- NA
  }
  if((inputs["no_insider"] == FALSE | is.na(inputs["no_insider"])) & (inputs["expbez"] == FALSE | is.na(inputs["expbez"]))){
    inputs["llm_expert"] <- NA
  }
  }
  for (variable in variables){
    if(variable %in% c("nameinst", "misclassification", "duplicate", "journalist", "passive_actor", "llm_quote", "expbez", "no_insider", "scidepend")){
  ...
  inputs["expert_status"] <- with(as.list(inputs), {case_when(
    llm_quote == FALSE | (llm_quote == TRUE & expbez == FALSE & is.na(llm_expert)) ~ 0,
    ((expbez == TRUE & llm_expert == 1) | (expbez == FALSE & no_insider == TRUE & llm_expert == 1)) & !(aktbez == 300 | (aktbez == 100 & scidepend == FALSE)) ~ 1,
    ((expbez == TRUE & llm_expert == 1) | (expbez == FALSE & no_insider == TRUE & llm_expert == 1)) & (aktbez == 300 | (aktbez == 100 & scidepend == FALSE)) ~ 2,
    (expbez == TRUE & llm_expert == 3) | (expbez == FALSE & no_insider == TRUE & llm_expert == 3) ~ 3,
    (expbez == TRUE & llm_expert == 2) | (expbez == FALSE & no_insider == TRUE & llm_expert == 2) ~ 4,
    (expbez == TRUE & llm_expert == 9) | (expbez == FALSE & no_insider == TRUE & llm_expert == 9) ~ 9
  )})
  full_dataset[full_dataset$entity_id == actor$entity_id, "expert_status"] <<- as.numeric(inputs["expert_status"])
...
set_to_last_value <- function(session = shiny::getDefaultReactiveDomain(), ids, index){
  ...
  for (variable in variables){
    if(is.na(actor_codes[[variable]][[1]])){
      if (variable %in% c("misclassification", "duplicate", "journalist", "passive_actor", "llm_quote", "expbez", "no_insider", "scidepend")){
  ...
  updateCheckboxInput(session = session, inputId = "scidepend", value = as.logical(actor_codes[["scidepend"]]))
  ...
  updateCheckboxInput(session = session, inputId = "llm_quote", value = as.logical(actor_codes[["llm_quote"]]))
  updateCheckboxInput(session = session, inputId = "expbez", value = as.logical(actor_codes[["expbez"]]))
  updateCheckboxInput(session = session, inputId = "no_insider", value = as.logical(actor_codes[["no_insider"]]))
  updateRadioButtons(session = session, inputId = "llm_expert", selected = actor_codes[["llm_expert"]][[1]])
}
...
reset_inputs <- function(session = shiny::getDefaultReactiveDomain()){
  ...
  updateCheckboxInput(session = session, inputId = "scidepend", value = FALSE)
  ...
  updateCheckboxInput(session = session, inputId = "llm_quote", value = FALSE)
  updateCheckboxInput(session = session, inputId = "expbez", value = FALSE)
  updateCheckboxInput(session = session, inputId = "no_insider", value = FALSE)
  updateRadioButtons(session = session, inputId = "llm_expert", selected = "")
}

## Markieren der Akteursnamen im Text
## Markieren von generischen Expertenbezeichnungen im Text

mark_actor_names <- function(actor_name){
  paste0("<mark style=\"background-color: LightSkyBlue;\"><strong>",actor_name,"</strong></mark>")
}
mark_expert_terms <- function(expert_term){
  paste0("<mark style=\"background-color: Lime;\"><strong>", expert_term, "</strong></mark>")
}
...
scidepend_input <- wellPanel(h6("Wissenschaftliche Abhängigkeit (SCIDEPEND)"),
                             checkboxInput("scidepend", label = "an Partialinteressen gebundene Forschung",
                                           value = FALSE))
...
llm_quote_input <- wellPanel(h6("LLM-Aussage (LLM)"),
                             checkboxInput("llm_quote", label = "Akteur:in äußert sich zu LLMs",
                                           value = FALSE))

expbez_input <- wellPanel(h6("Expert:innenbezeichnung (EXPBEZ)"),
                          checkboxInput("expbez", label = "Akteur:in wird als Expert:in bezeichnet",
                                      value = FALSE))

no_insider_input <- wellPanel(h6("Kein:e Insider:in (NOINSIDER)"),
                          checkboxInput("no_insider", label = "Akteur:in ist kein:e Insider:in",
                                        value = FALSE))

llm_expert_input <- radioButtons("llm_expert", label = "LLM-Expert:in (LLMEXP)",
                                 choices = c("Nichts ausgewählt" = "",
                                             "LLM-Entwicklung/Forschung" = 1,
                                             "LLM-Anwendung/Einsatz" = 2,
                                             "LLM-Folgen/Verortung" = 3,
                                             "Sonstiges" = 9,
                                             "Nicht erkennbar" = 99))
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      tabsetPanel(
        ...
        tabPanel(
          title = "Inhaltliche Kategorien 5 - LLM_QUOTE",
          fluidRow(column(8, includeMarkdown("source_codebuch/inhaltliche_variablen_5.md")))
        ),
        tabPanel(
          title = "Inhaltliche Kategorien 6 - EXPERT_STATUS",
          fluidRow(column(8, includeMarkdown("source_codebuch/inhaltliche_variablen_6_version2.md")))
        )
      )
    )
  )
      ...
      insertUI(selector = "#coder_info", where = "afterEnd",
               ui = tags$div(id = "inputs",
                             ...
                             conditionalPanel(
                               ...
                               llm_quote_input,
                               conditionalPanel(
                                 condition = "input.llm_quote == true",
                                 expbez_input),
                               conditionalPanel(
                                 condition = "input.llm_quote == true && input.expbez == false && (input.aktbez == 100 || input.aktbez == 200 || input.aktbez == 220 || input.aktbez == 300 || input.aktbez == 400 || input.aktbez == 500 || input.aktbez == 510 || input.aktbez == 520 || input.aktbez == 521 || input.aktbez == 522 || input.aktbez == 900 || input.aktbez == 910 || input.aktbez == 920 || input.aktbez == 930)",
                                 no_insider_input),
                               conditionalPanel(
                                 condition = "input.expbez == true || (input.expbez == false && input.no_insider == true)",
                                 llm_expert_input)),
                             hr(),
                             splitLayout(
                               cellWidths = c("50%","50%"),
...
## über den Kontext-Button ausgewählt wurden. Alle Vorkommen des Namens des Akteurs werden markiert.
  ...
  output$article <- renderUI({
    ...
    tags$div(id = "actor_text",
             ...
             br(),
             HTML(c("<p>", str_replace_all(paste0(text_shown, collapse = "</p><p>"), entity_name_surname, mark_actor_names) %>% 
                      str_replace_all(expert_terms, mark_expert_terms), "</p>")))
  })
...
## In einer Reactive-Expression werden die Werte der Eingabeelemente abgespeichert.
  ...
  codebogen <- reactive({
    rv$index_actor
    coded_data <- sapply(variables, function(x) ifelse(x %in% c("nameinst", "misclassification", "duplicate", "journalist", "passive_actor", "llm_quote", "expbez", "no_insider", "scidepend"), input[[x]], as.numeric(input[[x]])))
    coded_data
  })
...
## Wenn es noch weitere Akteure gibt, wird der Index des aktuellen Akteurs um 1 erhöht und die Kontext-Variable zurückgesetzt.
  ...
  observeEvent(input$submit_actor, {
    if(("" %in% c(input$nameinst, input$aktbez, input$aktort, input$gender, input$llm_quote) & !(input$misclassification | input$duplicate | input$journalist | input$passive_actor)) |
       (input$scidepend == "" & input$aktbez == 100) |
       (input$discipline == "" & input$aktbez %in% c(100, 300)) |
       (input$expbez == "" & !input$llm_quote == FALSE) |
       (input$no_insider == "" & (!input$expbez == TRUE | !input$aktbez %in% c(210, 230, 231, 232, 233, 234, 235, 236, 237))) |
       (input$llm_expert == "" & (input$expbez == TRUE | input$no_insider == TRUE))){
