...
public class WrapUtils {
    ...
    /*@ public normal_behaviour
      ...
      @
      @ // If there are more than len characters w/o linebreak,
      @ // then they are all different from space
      @ ensures (\forall int i; 0 <= i && i < s.length - len;
      @             (\forall int j; i <= j && j <= i + len; s[j] != '\n')
      @         ==> (\forall int j; i <= j && j <= i + len; s[j] != ' '));
      @
      ...
      @ assignable s[*];
      @*/
    public static void wrapLines(char[] s, int len) {
        ...
        /*@ loop_invariant -1 <= lastSpace && lastSpace < s.length;
          ...
          @ loop_invariant lastSpace <= lastChange + len + 1;
          @ loop_invariant lastSpace >= 0 ==>
          @          \old(s[lastSpace]) == ' ' || \old(s[lastSpace]) == '\n';
          @ loop_invariant (\forall int i; 0 <= i && i < s.length;
          @          s[i] == \old(s[i]) || \old(s[i]) == ' ' && s[i] == '\n');
          @ loop_invariant (\forall int i; 0 <= i && i < lastChange && i < s.length - len;
          @          (\forall int j; i <= j && j <= i + len; s[j] != '\n')
          @          ==> (\forall int j; i <= j && j <= i + len; s[j] != ' '));
        ...
        while (true) {
            ...
            if (nextSpace == -1) {
                if (s.length - lastChange > len
                        && (nextNewLine - lastChange > len || nextNewLine == -1)
                        && lastSpace >= 0) {
                    s[lastSpace] = '\n';
                }
                return;
            ...
            if (0 <= nextNewLine && nextNewLine < nextSpace) {
                if (nextNewLine - lastChange > len && lastSpace >= 0) {
                    s[lastSpace] = '\n';
                }
                lastChange = lastSpace = nextNewLine;
            } else {
                if (nextSpace - lastChange > len) {
    ...
    /*@ private normal_behaviour
      ...
      @ ensures \result == -1 ==> (\forall int i; from <= i && i < s.length; s[i] != c);
      @ ensures \result >= 0 ==>
      @                s[\result] == c && (\forall int i; from <= i && i < \result; s[i] != c);
