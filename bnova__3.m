...
4) Begin a Mathematica session with:    Needs["Tensor`bnova`"]
   ...
   12.03.2014  rotationTensor added
   10.04.2014 pickListPlot[] cycle[] step[] added
   25.10.2014  cross isotropy repaired c4 -> c4n and transfer99i added ( inverse to transfer99)
   29.10.2014 orthoE[] ixC[] added,  rotationTensor version Euler-Rodriguez repaired, pickListPlot[]  improved
   13.08.2014 corrected rotationTensor[f , t]  added rotation to xiC[mm] , added normalized[mm] in cross isotropic elasticities
   26.04.2017 careful with compositions like tpose24[ tpose23[ ]] , better use tpose[..,order],  zoutSym corrected
	 10.06.2017 declarations theReal[] and thePositiveReal[]  added as assumptions for Simplify
	 01.06.2018  tensorial[Sqrt, ...] special case, analytical solution
   29.08.2018  tensorial[Sqrt, ...],   tensorial[Exp, ...],   tensorial[Log, ...]   are now using built-in functions MatrixPower[..., 1/2], MatrixExp[  ]  and MatrixLog[...]
...
projOn::usage="projOn[a , b  ] returns projection of a onto b both being t3, t33,t333 or t3333 ";
restprojOn::usage="  restprojOn[a , b  ] returns the rest of a after projection of a onto b both being t3, t33,t333 or t3333
                    ...
                    and all three  being t3, t33,t333 or t3333 ";
...
theReal::usage = "Syntax: theReal[], theReal[a], or theReal[aList]  declares all Variables, a Variable or a List of Variables as Real \n via  Assumptions " ;
thePositiveReal::usage = "Syntax:  thePositiveReal[a], or thePositiveReal[aList]  declares  , a Variable or a List of Variables as Real and > 0 via  Assumptions " ;

st33::usage = "Syntax: st33[a]   \n similar to Array[a,{3,3}] but creates a symmetric  matrix \n  \
                 {{a[1, 1], a[1, 2], a[1, 3]}, {a[1, 2], a[2, 2], a[2, 3]},  {a[1, 3], a[2, 3], a[3, 3]}} ;  \n \
                  if a = {u,v,w} then DiagonalMatrix[{u,v,w}]
                  if a >0  then all components are RandomReal[-a,a] and symmetrized   " ;
...
approx::usage = "Syntax: approx[a,b] or  a ~approx~ b  \n        \
                 A logical function. If two arguments (numbers) are almost (10^-12) equal then returns True " ;
...
tr::usage = "Syntax: tr[a]  \n      returns the trace  of the  3x3 matrix a, i.e. a_ii. \
             Accepts also the diagonal form {a,b,c}"  ;
norm2::usage = "Syntax: norm2[a]  \n      returns the square of the norm of the  3x3 matrix a, \
                 i.e.  a_ij a_ij or of a vector a_i a_i. Accepts also the diagonal form {a,b,c}"  ;
...
symbolQ::usage = "Syntax: symbolQ[t_]:=\[Not]NumberQ[t] but works also with  lists and arrays. True means all components are symbolic  ";
...
antimetry4::usage = "Syntax:   antimetry4  \n  writes  the antimetry tensor 3333  identity4_ijkl = ( delta_ik delta_jl -  delta_il delta_jk ) /2 "   ;

identity4sym::usage = "Syntax:  unit4sym \n writes an identity tensor \n \
                      identity4sym_ijkl = ( delta_ik delta_jl + delta_il delta_jk )/2";
deviatorer::usage = "Syntax: deviatorer or deviatorer4 \n   returns a 3333-matrix  \n \
...
isomorphicP::usage = "Syntax:  RoscoeP[x] \n  calculates isomorphic invariant  P = -tr[x]/Sqrt[3] of a tensor x. \
                  Accepts also the diagonal form x={a,b,c}.  Argument x must have mechanical signs \n
                   (tension positive)  " ;
isomorphicQ::usage = "Syntax:  isomorphicQ[a]  \n  calculates isomorphic  invariant Q  = Sqrt[norm2[deviator[a]]]. \
                  Accepts also the diagonal form x={a,b,c} and  gives sign in the axisymmetric case x={a,b,b} " ;
onev::usage = "onev normalized second rank identity tensor, e.g. P = -onev ~colon~ T gives isomorphic pressure, see also onevstar " ;
onevstar::usage = " onevstar  normalized second rank deviatoric tensor, e.g. Q =  onevstar ~colon~ T gives isomorphic deviarotic stress (Q  with sign). \
                     see also onev " ;
...
LodeTheta::usage = "Syntax:  LodeTheta[a] \n calculates Lode angle  \n \
                     theta  = (1/3) * ArcCos[ -3*Sqrt[3]*j3[x]/(2* (j2[x])^(3/2)  )].\
                      Accepts also the diagonal form {a,b,c}   Argument x must have mechanical signs \n
                    (tension positive) " ;
tensorial::usage = "Syntax:  tensorial[func, a] \n performs func  on the eigenvalues \n \
                     e.g. tensorial[#^3 &, a] or tensorial[Sqrt, aa] or tensorial[Log, aa]  " ;
tensorialfD::usage = "Syntax  tensorialfD[func, A] \n  returns Frechet derivative of func(A) wrt tensor A  \n \
                     e.g. tensorialfD[#^3 &, aa] or tensorialfD[Sqrt, aa] or tensorialfD[Log, aa]  \n \
                      example: b=  tensorial[Exp, a ] ; tensorialfD[ Log, b] ~colon~ tensorialfD[Exp, a] //transfer99//Chop //MatrixForm " ;
decomposeVRU::usage = "Syntax  decomposeVRU[F]  \n polar decomposition of  F,  \n e.g.  {v,r,u} =  decomposeVRU[F]  " ;
inverse99::usage = "Syntax:  inverse99[a99] inverts a 9x9 or 3x3x3x3 NUMERICAL matrix \
...
iE33::usage = "Syntax  iE33[e,n]  or iE[ ] \n  returns a 33-matrix in the form \
                     of isotropic elastic stiffness for principal components, see also iC33[ ],  iE[] and iC[]" ;
iC33::usage = "Syntax  iC33[e,n]  or iC[ ]  \n  returns a 33-matrix in the form \
                     of isotropic elastic compliance for principal components, see also iE33[ ],  iE[] and iC[]" ;
...
isoPQ::usage = "Syntax: isoPQ[A] returns isomorphic PQ components of A. \n \
                If A is t33Q  or t3Q tensor then {AP,AQ}  is returned (mech. signs assumed in original A). \n \
                If A is t3333Q then the matrix {{APP,APQ},{AQP,AQQ}} is returned  \n \
                see also isoPQgeot[]  " ;
isoPQgeot::usage = "Syntax: isoPQgeot[A] returns isomorphic PQ components of A. Geotechnical signs assumed in original A. \n\
                   If A is t33Q  or t3Q tensor then {AP,AQ} is returned \n\
                   If A is t3333Q  then the matrix {{APP,APQ},{AQP,AQQ}} is returned \n\
                   see also isoPQ[]  " ;
...
symm::usage = "Syntax: symm12[a],symm34[a],symm23[a],symm14[a],symm24[a],symm13[a] or symm13i24[a]. \n  \
     ...
     For 2nd order tensor sym12[a] = symm[a].  Elastic minor and major symmetries can be imposed by symmEl[a]" ;
...
rotateTensor::usage = "Syntax:  rotateTensor[T, a] rotates tensor T   of rank 1 to 4 (t3, t33, t333 or t3333) using a 33 rotation matrix a.
    Summation is performed over the second index of a, e.g.  T'_ij  = a_ik a_jl  T_kl.  Directional cosine matrix a can be
    calculated with aRot for active rotation or pRot for passive rotation";


rotationTensor::usage=" rotationTensor[f_?t3Q, t_?t3Q ] or rotationTensor[f_?t3Q, t_?t3Q, \[Beta]_ ] or
  rotationTensor[f_?t33Q, t_?t33Q ] or rotationTensor[f_?t33Q, t_?t33Q, \[Beta]_ ] \n\
returns a rotation tensor R  for   t=R.f  or t=R~colon~f or its portion defined by  angle \[Beta] in radians.
...
xiEgeot::usage = "Syntax: xiEgeot[ Ev, nh, a, mm ]  or   xiEgeot[ Ev, nh, a, n, mm ]    returns a simplified (geotechnical) 3333 cross-isotropic
            stiffness with axial isotropy wrt mm according to Graham and Houlsby's  paper in Geotechnique 1983,
            possibly  enhanced with exponent n,  index h (=horizontal) perpendicular to sedimentation mm,  index v (= vertical) along sedimentation mm " ;
...
xiCgeotShort::usage = "Syntax:   xiCgeotShort[ Ev_, nh_, a_, mm_?t3Q  ]	xiCgeotShort[ Ev_, nh_, a_, n_, mm_?t3Q  ] returns a simplified (geotechnical) 3333 cross-isotropic
           compliance with axial isotropy wrt mm according to Graham and Houlsby's  paper in Geotechnique 1983,
            possibly  enhanced with exponent n, index h (=horizontal) perpendicular to sedimentation mm,  index v (= vertical) along sedimentation mm " ;



xiEgeotShort::usage = "Syntax:       xiEgeotShort[ Ev_, nh_, a_, mm_?t3Q  ] or	xiEgeotShort[ Ev_, nh_, a_, n_, mm_?t3Q  ]  returns a simplified (geotechnical) 3333 cross-isotropic
            stiffness with axial isotropy wrt mm according to Graham and Houlsby's  paper in Geotechnique 1983,
            possibly  enhanced with exponent n,  index h (=horizontal) perpendicular to sedimentation mm,  index v (= vertical) along sedimentation mm
            It is identical as xiEgeot but uses xiEr ";


getXAniso::usage = "Syntax getXAniso[a,mm] or getXAniso[a,n,mm]  returns (geotechnical) 3333 anisotropy tensor with axial isotropy wrt mm according to Graham and Houlsby \n \
              or  enhanced with exponent n for scaling of a stiffness matrix" ;
...
pqTriaxPlot::usage = "Syntax:  pqTriaxPlot[path_, sfi_:0] produces  plots of stress and strain paths in p-q and eps_vol - eps_q planes and lines for M_C and M_E if sfi > 0. \n \
                    The optional parameter sfi  = sin \[Phi]_c  is by default sfi = 0. Only triaxial states are considered. Values q and eps_q may be negative.
                    Transpose[path]  = {pathT, pathEps, ... } should contain a list of states with at least two components being full stress and strain tensors
                     see also   deviatoricPlot, isoTriaxPlot, pradhanTriaxPlot " ;

deviatoricPlot::usage = "Syntax: deviatoricPlot[path_, sfi , pMN  ] produces deviatoric plot of the stress path.
                    The optional parameter sfi  = sin \[Phi]_c  is by default sfi = 30\[Degree] is used to plot the Matsuoka Nakai criterion
                    for the mean stress (optional parameter) pMN. By default pMN = 100.  Tension planes are plotted too.
                    Transpose[path]  = {pathT, pathEps, ... } should contain a list of states with at least two components being full stress and strain tensors
                     see also   pqTriaxPlot,    isoTriaxPlot, pradhanTriaxPlot "  ;

isoTriaxPlot::usage = "Syntax:  isoTriaxPlot[path, sfi] produces  plots of stress and strain paths in isomorphic P-Q and eps_P - eps_Q planes and lines for M_C and M_E if sfi > 0 \n \
                    The optional parameter sfi  = sin \[Phi]_c  is by default sfi = 0. Only triaxial states are considered. Values q and eps_q may be negative.
                    Transpose[path]  = {pathT, pathEps, ... } should contain a list of states with at least two components being full stress and strain tensors
...
matrixDiagonal::usage = "Syntax: matrixDiagonal[T] if  called with an argument  t33Q returns a list of diagonal components (opposite to DiagonalMatrix[]).
...
step::usage = "Syntax:  step[ mat,  loading, ninc] is a loop calling  increment[mat,  loading ] and appending the updated states (returned by increment[ ] )
to states it is assumed that the constitutive routine increment[mat,  loading ]  reads the current state using   {...} = Last[ states ].
 loading describes a constant load increase in each increment. Similar functions:  parametricStep[] or cycle[ ] " ;

parametricStep::usage = "Syntax: parametricStep[ mat_,  loading_,  iinc_ , ninc_]  is a loop calling  increment[mat,  loading ] and appending
the updated states (returned by increment[ ] )  to states it is assumed that the constitutive routine increment[mat,  loading ]
reads the current state using   {...} = Last[ states ].  loading[iinc] is a user's function that describes a load increase in increment iinc.
Similar functions: step[] and cycle[]." ;

cycle::usage = "Syntax:  cycle[mat_,  loading1_ ,ninc1_,loading2_ ,ninc2_,ncyc_] is a loop calling  twice step[ ] per cycle
it is assumed that the constitutive routine increment[mat,  loading ]  reads the current state using   {...} = Last[ states ] ";

increment::usage="User's routine  to calculate  an updated state. It is used as  AppendTo[states, increment[mat, loading, states ] ] " ;

states::usage = "A global variable used inside the users' routine increment[ ] as follows: state = Last[states]; or AppendTo[states, updatedstate ]; ";

mat::usage="list of material parameters for step[] cycle[], etc.";
loading::usage="loading parameters for step[]  "  ;  loading1::usage="loading parameters for  cycle[] "  ;loading2::usage="loading parameters for cycle[] "  ;
states::usage="global variable with history of states for step[] cycle[], etc."  ;
ninc::usage="number of increments for step[] cycle[] "  ; ninc1::usage="number of increments for  cycle[] "  ; ninc2::usage="number of increments for  cycle[] "  ;
ncyc::usage="number of cycles for cycle[] "   ;
\[Nu]::usage=" \[Nu] is the Poisson number as a global variable ";
...
voigtE[ e_?t3333Q ] :=   Module[ {a,aux,aux1,aux2,aux3,out},
a = transfer99[e];
...
out = Array[0 & ,{6,6}] ;
out[[1;;4,1;;3]] = a[[1;;4,1;;3]];
out[[1;;4,4;;6]] = aux[[1;;4,1;;3]];
out[[5, 1;;3 ]] = a[[6,1;;3 ]];
out[[5, 4;;6 ]] = aux[[6,1;;3 ]];
out[[6, 1;;3 ]] = a[[8,1;;3 ]];
out[[6, 4;;6 ]] = aux[[8,1;;3 ]];
...
voigtEi[ ee_?t66Q ] :=   Module[ {c1,c2,c3, c4,c5,c6,r1,r2,r3, r4,r5,r6, aux,aux1,aux2,aux3,out},
{c1,c2,c3, c4,c5,c6}= Transpose[ee] ;  (* makes the columns 4,5,6 double *)
{r1,r2,r3, r4,r5,r6} = Transpose[ {c1,c2,c3,c4,c4,c5,c5,c6,c6}];
out = {r1,r2,r3, r4,r4,r5,r5,r6,r6}  ; (* makes the rows 4,5,6 double *)
transfer99i[out]
];



voigtC[ e_?t3333Q ] :=   Module[ {a,aux,aux1,aux2,aux3,out},
a = transfer99[e];
...
aux = Transpose[{aux1,aux2,aux3}];
out = Array[0 & ,{6,6}] ;
out[[1;;3,1;;3]] = a[[1;;3,1;;3]];
out[[1;;3,4;;6]] = aux[[1;;3,1;;3]];
out[[4,1;;3]] = a[[4,1;;3]] +  a[[5,1;;3]] ;
out[[4,4;;6]] = aux[[4,1;;3]] +  aux[[5,1;;3]] ;
out[[5, 1;;3 ]] = a[[6,1;;3 ]] +  a[[7,1;;3 ]]  ;
out[[5, 4;;6 ]] = aux[[6,1;;3 ]] + aux[[7,1;;3 ]] ;
out[[6, 1;;3 ]] = a[[8,1;;3 ]]  +  a[[9,1;;3 ]] ;
out[[6, 4;;6 ]] = aux[[8,1;;3 ]] + aux[[9,1;;3 ]];
out
];

voigtCi[ ee_?t66Q ] :=   Module[ {c1,c2,c3, c4,c5,c6,r1,r2,r3, r4,r5,r6, aux,aux1,aux2,aux3,out},
{c1,c2,c3, c4,c5,c6}= Transpose[ee] ;  (* makes the columns 4,5,6 double *)
{r1,r2,r3, r4,r5,r6} = Transpose[ {c1,c2,c3,c4,c4,c5,c5,c6,c6}];
out = {r1,r2,r3, r4/2,r4/2,r5/2,r5/2,r6/2,r6/2}  ; (* makes the rows 4,5,6 double *)
transfer99i[out]
];
...
iE33[e_,n_] :=  Module[ {a,b}, a=e*(1-n)/((1+n)*(1-2*n)); b = e* n/((1+n)*(1-2*n));  {{a,b,b},{b,a,b},{b,b,a}}  //Simplify ];
iE33[ ] :=  Module[ {a,b}, a=e*(1-\[Nu])/((1+\[Nu])*(1-2*\[Nu])); b = e* \[Nu]/((1+\[Nu])*(1-2*\[Nu]));  {{a,b,b},{b,a,b},{b,b,a}}  //Simplify ];
iC33[e_, n_] := {{1,-n,-n},{-n,1,-n},{-n,-n,1}} /e;
iC33[ ] := {{1,-\[Nu],-\[Nu]},{-\[Nu],1,-\[Nu]},{-\[Nu],-\[Nu],1}} /e;
iE[e_,n_]:=Module[{G,K}, G=e/(2*(1+n)); K=e /(3*(1-2*n));    3K*(onev ~out~ onev)   + 2G*  deviatorer4sym    //Simplify   ] ;
iC[e_,n_]:=Module[{G,K}, G=e/(2*(1+n)); K=e /(3*(1-2*n)) ;   1/(3K)*(onev ~out~ onev)   + 1/(2G)* deviatorer4sym   //Simplify   ];
iE[ ]:=Module[{G,K}, G=e/(2*(1+\[Nu])); K=e /(3*(1-2*\[Nu]));    3K*(onev ~out~ onev)   + 2G*  deviatorer4sym    //Simplify   ];
iC[ ]:=Module[{G,K}, G=e/(2*(1+\[Nu])); K=e /(3*(1-2*\[Nu])) ;   1/(3K)*(onev ~out~ onev)   + 1/(2G)* deviatorer4sym   //Simplify  ];
...
xiC[] :=   Module[{o11,o12,o13,o22,o23,o33,o44,o66,o88,cc99,cc,nvh,Gh},
 Message[bnova::"Globals", "{Eh,Ev,nh,nhv,Gv}=", {Eh,Ev,nh,nhv,Gv} ] ;
 Gh = Eh/(2+2 nh) ;
 nvh = nhv Ev / Eh ;
o11 =   1/Eh ;
o12 = -nh/Eh;
...
xiEgeotShort[ Ev_, nh_, a_, n_, mm_?t3Q  ]:= Module[{c1,c2,c3,c4,c4n,c5,c6,mm2 , Eh, Gv, Gh, nvh, nhv, g, mmm}, (* simplified (Ev nh a, n) el. parameters  & given dir. mm  *)
         mmm = normalized[mm];
         Eh = Ev*a^(2/n);
         Gh = Eh/(2+2 nh) ;
         Gv = Gh / a;
...
getXAniso[a_,m_?t3Q]:= Module[{mm,mu,Q}, (* simplified a el. parameters & given dir. mm  *)
    mm=m/Norm[m];
    mu = Sqrt[a] delta + (1-Sqrt[a]) mm~out~mm;
    Q =tpose23[mu~out~ mu ]  //Simplify
];

getXAniso[a_,n_,m_?t3Q]:= Module[{mm,mu,Q,d,aa,b}, (* simplified a el. parameters & given dir. mm  *)
    mm=m/Norm[m];
    aa = -(Sqrt[a^(2*n)*((-1 + Sqrt[a])^2 + 2*a^(-1/2 + n) + (-3 + a)*a^n)]/Sqrt[a + (-4 + a)*a^(2*n) + 2*a^(1 + n)]);
    b = (Sqrt[a^(2*n)*((-1 + Sqrt[a])^2 + 2*a^(-1/2 + n) + (-3 + a)*a^n)]*(Sqrt[a] + a - 2*a^n - a^(1/2 + n) + a^(1 + n)))/((-1 + a)*a^n*Sqrt[a + (-4 + a)*a^(2*n) + 2*a^(1 + n)]);
    d = (a^(-1/2 + n)*(a - a^n)*(Sqrt[a] + 2*a^n + a^(1/2 + n)))/(a + (-4 + a)*a^(2*n) + 2*a^(1 + n));
...
cycle[mat_,  loading1_ ,ninc1_, loading2_, ninc2_, ncyc_]  :=  Module[{icyc},  Do[ step[mat, loading1 ,ninc1] ;   step[mat, loading2, ninc2 ] ,{icyc,1,ncyc}] ;    ];
...
deviatoricPlot[path_, sfi_:30\[Degree] , pMN_:100  ] := Module[
{pathT,pathEps,  pathTstar,   devIxy  , pathxy , rmax, gaxes, gminusAxes, glabels,x,y,tt, gMN, gnoTens, gpath},
{pathT, pathEps} = Transpose[path ][[1;;2]];     (* stress plot with M-N criterion at pMN =100 kPa *)
devIxy =  {{0,2},{Sqrt[3],-1},{-Sqrt[3],-1}}/2 ;
xyIdev =  {{1, 2, 0}/Sqrt[3], {1, 0, 0}};
pathTstar = deviator[#] & /@ pathT;
y =    matrixDiagonal[ # ][[1]]  &   /@  pathTstar;
x =    ((  2*matrixDiagonal[ # ][[2]] & /@  pathTstar)  + y) /Sqrt[3];
pathxy = -{x,y} //Transpose;     (*geot. sign convention *)
x=.;y=.;
rmax  = Max[Norm[#] & /@  pathxy];
...
tt = { -y - pMN,-(1/2) (Sqrt[3] x-y)-pMN, 1/2 ( Sqrt[3] x+y)-pMN}  ;
gMN = RegionPlot[ tr[tt]*tr[1/tt]  <   (9-sfi^2)/(1-sfi^2)   && x^2 + y^2 < pMN^2, {x, -0.8*pMN, 0.8*pMN },  {y, -0.8*pMN, 0.9*pMN }];
gnoTens = RegionPlot[ tt[[1]] < 0 && tt[[2]] < 0 && tt[[3]] < 0, {x, -2.2*pMN, 2.2*pMN },  {y, -3.2*pMN, 2.2*pMN }, PlotStyle->None ];
 gpath= ListPlot[pathxy, AspectRatio -> Automatic];
Show[gMN, gpath,gnoTens,   Graphics[ gaxes ] , Graphics[gminusAxes]  , Graphics[glabels]]
];
...
isoPQ[X_?t3Q]:={(-DiagonalMatrix[X]~colon~onev),(DiagonalMatrix[X]~colon~onevstar)};
isoPQ[X_?t33Q]:={-(X~colon~onev),(X~colon~onevstar)};
...
isoPQgeot[X_?t3Q]:={(DiagonalMatrix[X]~colon~onev),(DiagonalMatrix[X]~colon~(-onevstar))};
isoPQgeot[X_?t33Q]:={(X~colon~onev),(X~colon~(-onevstar))};
...
isomorphicQ[{b_,a_,b_}] := -Sqrt[2/3](a-b);   (*  x = {a, b, b}; y = {c, d, d}; x.y  - isomorphicP[x] isomorphicP[y] - isomorphicQ[x] isomorphicQ[y] // Simplify *)
...
matrixForm[mm_?t3333Q]:= Module[ {i,j, i9={1,2,3,1,2,1,3,2,3}, j9={1,2,3,2,1,3,1,3,2}, aa=Array[0&, {9,9}]  },
 ...
 transfer99i[mm_?t9Q]:= Module[ {i, i9={1,2,3,1,2,1,3,2,3} , j9={1,2,3,2,1,3,1,3,2},  aa=Array[0&,9 ]  },
                          Do[mm[[   i9[[i]],  j9[[i]]    ]] = aa[[i]], {i,1,9}  ]; aa  ]  ;
...
tensorial[f_, {{a_, 0, b}, {0, d_, 0}, {b_, 0, c}}   ] :=  Module[ {L1,L2,R,rule,a11,a12,a21,a22},
R =  Array[0 &, {3, 3}];
...
tensorial[f_, {{d_, 0, 0}, {0, a_, b_}, {0, b_, c_}}   ] :=  Module[ {L1,L2,R,rule},
R =  Array[0 &, {3, 3}];
...
decomposeVRU[f_] := Module[{v=Array[0 &, 3],r=Array[0 &, {3,3}],u=Array[0 &, {3,3}]},
                                 v = f.Transpose[f]; v = tensorial[Sqrt,v];
                                 u = Transpose[f].f; u = tensorial[Sqrt,u];
                                 r=f.Inverse[u]; {v,r,u}];
...
Protect[ antimetry4, approx,aRot,aRot12,aRot13,aRot23,atomSymbolQ,axissymQ,axisymm23Q,coaxialQ,colon,colon3,colon4,contract,convertCart2Cyl,convertCart2Spher,convertCyl2Cart,convertSpher2Cart,cycle,
decomposeVRU,delta,dev,deviator,deviatorer,deviatorer4,deviatorer4sym,deviatoricContourPlot,deviatoricPlot,deviatoricRangePlot,doubleCrossProduct,externalHashProduct,getXAniso,givensRotate,
hatcolon,hated,hausholderMatrix,i1,i2,i3,iC,iC33,iCGehring,iCNiemunis,iCVermeer,identity4,identity4sym,iE,iE33,iEHoulsby,iELiu,iEVermeer,inverse99,inversedSM,inverseSM,isomorphicP,isomorphicQ,isoPQ,isoTriaxPlot,
j2,j3,LodeTheta,matrixDiagonal,matrixForm,norm2,normalized, numberQ,numericQ,onev,onevstar,orthoC,orthoE,out,pickListPlot,pqTriaxPlot,pradhanTriaxPlot,projOn,pRot,pRot12,pRot13,pRot23,qubic,
realQ,restprojOn,Ricci,RoscoeEpsilonQ,RoscoeEpsilonV,RoscoeP,RoscoeQ,rotateTensor,rotationTensor,scalar,squareMQ,st33,step,stressResponse3D,stressResponse3Dstar,stressResponsePQ,stressResponsePstarQstar,
symbolQ,symm23,symmEl,symmMQ,t2Q,T2xyp,t33333333Q,t33Q,t3Q,tensorial,tensorialfD,thePositiveReal,theReal,tpose,tpose12,tpose13,tpose13i24,tpose14,tpose23,tpose24,tpose34,tQ,tr,transfer99,transfer99i,
vectorQ,voigtC,voigtCi,voigtE,voigtEi,voigtEps,voigtEpsi,voigtSig,voigtSigi,xiC,xiCgeotShort,xiCr,xiE,xiEgeot,xiEgeotShort,xiEr,xyp2T,zout,zoutSym
 ];



Print[ " You  are in the context bnova. It provides:
  SMALL OBJECTS :   st33  Ricci  delta  onev  onevstar aRot pRot rotationTensor
  ...
  INVARIANTS :      i1  i2  i3  j2  j3 RoscoeP RoscoeEpsilonV RoscoeQ RoscoeEpsilonQ  LodeTheta isomorphicP  isomorphicQ
  TRANSPOSITIONS:   tpose, tpose12  tpose34  tpose23 tpose14  tpose24  tpose13  tpose13i24
  4-TH ORDER TENSORS:  ISOTROPIC:    iE33 iC33 iE iC  identity4sym   deviatorer deviatorer4sym  antimetry4
                       CROSS-ANISO:   xiE  xiC   xiEr  xiCr  xiEgeot xiCgeotShort xiEgeotShort
                       ORTHOTROPIC:   orthoE orthoC  iEVermeer iCVermeer identity4  iCNiemunis  iCGehring  getXAniso  iEHoulsby iELiu
  TENS. MANIPULATIONS:  isoPQ tensorial tensorialfD decomposeVRU inverse99 inverseSM  inversedSM
                        matrixForm  transfer99 transfer99i voigtEps voigtEpsi voigtSig voigtSigi voigtE voigtEi voigtC voigtCi
                       convertCart2Spher  convertSpher2Cart convertCart2Cyl convertCyl2Cart
  ELEMENT TESTS   :  step,cycle, increment (=user's routine)
  PLOTS:             pradhanTriaxPlot pqTriaxPlot deviatoricPlot isoTriaxPlot stressResponsePQ  stressResponsePstarQstar stressResponse3D  stressResponse3Dstar
                      pickListPlot,  deviatoricContourPlot  deviatoricRangePlot T2xyp xyp2T
  AUXILIARIES:        approx zout zoutSym ~zout~ ~zoutSym~ decomposeVRU ~hatcolon~   symm23 symmEl  matrixDiagonal
  QUERIES:            realQ numberQ symbolQ tQ t2Q t3Q t33Q ...  t33333333Q   numericQ  atomSymbolQ coaxialQ axissymQ vectorQ squareMQ  symmMQ
