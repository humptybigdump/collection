...
%the truncation error Rn is computed below
...
%%note : for f(x)=exp(1).^x, fd1=fd2=..=fdn=f(x); so that
...
% here, in the first loop we'll use np(1) = 10+1 data points
% second loop will be np(2) = 100+1 data points, and so on ...

for k=1:length(np);
    % generate the sample points (xs,ys)
    % with resolution defined by np
    %xs = single(linspace(x0,xn,np(k))); % using single precision
    xs = (linspace(x0,xn,np(k)));         % default is double precision
    ys = (f(xs));

    % determine h=(b-a)/(nr of segments)
    % remember!! segments = (number of points - 1)
    h(k)= (xs(end)-xs(1))/(np(k)-1); 
    
    % compute area under curve
    % function(s) are at the end of this script
    %area_t(k) = mytrap(xs,ys);
    %area_matlab(k) = trapz(xs,ys); %use built-in
    area(k) = simpson(xs,ys);
end
%
% Errors
error =exact-area;
...
% The truncation error (determined analytically!!!)
