...
% sort HQ values from small to large values ("sorted_HQ")
sorted_HQ = sort(HQ);
...
PE = (1:length(HQ)) / N; % frequencies of observed values 
...
normal_cdf_fit = fitdist(sorted_HQ,'Normal'); % function to fit a normal cdf to HQ data to find mean and standard deviation as input for generating a normal cdf
...
sorted_HQ_STD = normal_cdf_fit.sigma;  % standard deviation as first input parameter for creating normal cdf
