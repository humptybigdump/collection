...
isoPQ::usage = "Syntax: isoPQ[A] returns isometric PQ components of  A. \n \
                If A is an axisymmetric mechanical tensor t33Q  or t3Q  of the form {a,b,b} then {AP,AQ}  is returned (mech. signs assumed in original A). \n \
                If A is tensor t3333Q that interrelates axisymmetric mechanical tensors of the form {a,b,b} then the matrix {{APP,APQ},{AQP,AQQ}} is returned  \n \
                see also isoPQgeot[]  " ;
isoPQgeot::usage = "Syntax: isoPQgeot[A] returns isometric PQ components of A. Geotechnical signs assumed in original A. \n\
                     If A is an axisymmetric geotechnical tensor t33Q  or t3Q  of the form {a,b,b} then {AP,AQ}  is returned (mech. signs assumed in original A). \n \
                     If A is tensor t3333Q that interrelates axisymmetric geotechnical tensors of the form {a,b,b} then the matrix {{APP,APQ},{AQP,AQQ}} is returned  \n \
                   see also isoPQ[]  " ;
...
isoTriaxPlot::usage = "Syntax:  isoTriaxPlot[path, sfi] produces  plots of stress and strain paths in isometric P-Q and eps_P - eps_Q planes and lines for M_C and M_E if sfi > 0 \n \
                    The optional parameter sfi  = sin \[Phi]_c  is by default sfi = 0. Only triaxial states are considered. Values q and eps_q may be negative.
                    Transpose[path]  = {pathT, pathEps, ... } should contain a list of states with at least two components being full stress and strain tensors
...
increment::usage="User's routine  to calculate  an updated state. It is used as  AppendTo[states, increment[mat, loading ] ] . 
           The first line is the {eps, sig, ...} = states ; de = loading;  and after update the last  line   {esp, sig, ...}  ] ]"  ;
...
isoPQ[X_?t3Q]:= If[ X[[3]] == X[[2]],   {(-DiagonalMatrix[X]~colon~onev),(DiagonalMatrix[X]~colon~onevstar)} , Print[ "Error in isoPQ: argument  ",X, "  is not of the form {a,b,b}" ], 
  Print[ "Error in isoPQ: argument  ",X, "  is not necessarily of the form {a,b,b}" ]  ]; 
isoPQ[X_?t33Q]:=  If[ DiagonalMatrixQ[X] && X[[3,3]] == X[[2,2]] ,   {-(X~colon~onev),(X~colon~onevstar)} ,   Print[ "Error in isoPQ: argument  ",X, "  is not of the form DiagonalMatrix[{a,b,b}]" ], 
  Print[ "Error in isoPQ: argument  ",X, "  is not necessarily of the form DiagonalMatrix[{a,b,b}]" ]
  ]; 
...
hatcolon,hated,hausholderMatrix,i1,i2,i3,iC,iC33,iCGehring,iCNiemunis,iCVermeer,identity4,identity4sym,iE,iE33,iEHoulsby,iELiu,iEVermeer,inverse99,inversedSM,inverseSM,isomorphicP,isomorphicQ, isoP,isoQ,    isoPQ,isoTriaxPlot,
