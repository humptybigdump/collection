...
# Erst Pegel Oberstdorf, unten dann Bechhofen
 ...
 ### Daten einlesen
 #  Oberstdorf
  obst <- read.csv("1.5_Wasserbilanz_OBST.csv", skip=7, colClasses=c("character","character","numeric","numeric"), sep=";")
  ...
  plot(cumsum(N)~Datum, obst, t="l", col=4, ylab="Kumuliert/mm", main="Oberstdorf/Stillach")
   lines(cumsum(Q.mm) ~Datum, obst, col=6)
 ...
 #  Bechhofen
  ...
  bech <- read.csv("Wasserbilanz_bech.csv", skip=7, colClasses=c("character","character","numeric","numeric"), sep=";")
  ...
  # in hydrologische Jahre zerlegen  - laut Din ab 1.11.
  bech$hj <- year(bech$Datum)
  bech$hj[month(bech$Datum) %in% c(11,12)] <- bech$hj[month(bech$Datum) %in% c(11,12)]+1

  plot(cumsum(N)~Datum, bech); lines(cumsum(Q.mm)~Datum, bech, col=3)
