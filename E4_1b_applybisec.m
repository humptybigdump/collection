...
% Tolerance of the error of stopping criterion...
% in the example bisection.m, ...
% we chose (abs(f(xr)) < tolerance) as stopping criterion.
et=1e-5;
...
matlab_root1=fzero(@(x) -0.5*x.^2 + 2.5*x + 4.5,[-3 0])
matlab_root2=fzero(@(x) -0.5*x.^2 + 2.5*x + 4.5,[5 10])
