...
%% Task 1: Calculate Evapotranspiration ET from the lysimeter data by closing the water balance (0 = P - Q - S - ET)
...
e_s = 6.11 * exp((17.62 * temp_14) ./ (243.12 + temp_14));
    
ET_Haude = f .* e_s .* (1 - (rel_sat ./ 100));
...
RMSE_Haude = sqrt(mean((act_ET - ET_Haude) .^2))
...
ET_Turc = 0.0031 .* C .* (global_radiation + 209) .* (temp ./ (temp + 15));
...
RMSE_Turc = sqrt(mean((act_ET - ET_Turc).^2))
