...
private:
    ...
    bool handleOneInclude(const GLchar* path, GLchar** shader, bool unscramble = false) {
        char* tok = (char *) *shader;
        tok = strstr(tok, "#include");

        char dummy[1024], incFile[1024];
        ...
        sscanf(tok, "%s %s", dummy, incFile);

        // note: start at incFile[1] (skip ") and replace 2nd " by 0
        for (size_t i = 1; i < strlen(incFile); i++)
            if (incFile[i] == '"')
                incFile[i - 1] = 0;
            else
                incFile[i - 1] = incFile[i];

        int skipLen = (int) (strlen(dummy) + strlen(incFile));
        while (tok[skipLen] != '"') skipLen++;
        ...
        if (f == NULL) {
            printf("[GLSL]: %c> '%s' not found, trying GLF's '%s'.\n", 192, filename, incFile);
            sprintf(filename, "./GLF/shader/%s", incFile);
            f = fopen(filename, "rb");
        }

        if (f == NULL) {
            printf("[GLSL]: %c> could not open include file '%s'\n", 192, filename);
            exit(1);
        } else
            printf("[GLSL]: %c> including '%s'\n", 192, filename);

        fseek(f, 0, SEEK_END);
        int len = ftell(f);
        fseek(f, 0, SEEK_SET);
        GLchar* incSrc = new GLchar[len + 1];
        fread(incSrc, 1, len, f);
        incSrc[len] = 0;

        if (unscramble)
            unscrambleSource(incSrc, len);

        int oldLength = (int) strlen(*shader);
        int newLength = oldLength + len + 1024;
        GLchar* temp = new GLchar[newLength + 1];
        memset(temp, 0, newLength);

        int p = 0;
        for (int i = 0; i < tok - (*shader); i++)
            temp[p++] = (*shader)[i];

        for (int i = 0; i < len; i++)
            temp[p++] = incSrc[i];

        for (int i = skipLen + (int) (tok - *shader) + 1; i < oldLength; i++)
            temp[p++] = (*shader)[i];
