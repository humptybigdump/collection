...
getIntersectionPointBetweenSegments::usage= " getIntersectionPointBetweenSegments[seg1_, seg2_] returns the intersection point 
                                             of lines defined by segments seg1 = {{x1,y1},{x2,y2}} and seg2 = {{x3,y3},{x4,y4}} ";

getOverlapLength::usage=" getOverlapLength[seg1,seg2]  returns the length of the overlap of two 1D segments   " ; 

clearGlobals::usage=" clearGlobals[]; 
 undefines global variables (from topology kinematics and statics) which may have been left by the previous run of getHodograph etc. 
 OPTIONS:
 verbose -> True
 ";


getTopology::usage = "{edgesT,edgesN,edgesL,adjacentE,areasE} = getTopology[elements, edges, xx] 
calculates auxiliary geometrical data 
ARGUMENTS  : 
 elements = lists with node numbers of elements 
 edges = pairs with node numbers on each slip line 
 xx = list pairs with coordinates of nodes 

 Returned lists  are :  
 edgesT = list of unit  tangent vectors  for  for all edges: ,  
 edgesN = list of unit   normal vectors  for  for all edges:   
 edgesL  = list of lengths   for  for all edges: 
 adjacentE  = list of pairs with adjacent element numbers left and right (looking along edgesT)  
 areasE =   :list of areas   for all elements 
...
plotKEM::usage = "plotKEM[ elements,edges,xx,v,opts]   
draws  two meshs with KEM elements: before and after the displacement
ARGUMENTS : 
  elements = lists with node numbers of elements 
 edges = pairs with node numbers on each slip line 
 xx = list pairs with coordinates of nodes 
 v = displacements of elements used to show the shifted geometry 
 
 OPTIONS :  
 nodeLabels->True       edges, nodes and elements may be numbered
 elementLabels->True
 edgeLabels->True 
...
plotForcePolygon::usage = "plotForcePolygon[ elementNumber, solution, options] 
draws the polygon of forces for the body elementNumber 
ARGUMENTS : 
 elementNumber = number of element for which the graphic should be plotted 
 solution = list of rules to evaluate the  unknown inter-element forces q[i] obtained from  {solution} = Solve[staticEquilibriumEquations,unknowns]
 OPTIONS :  
 imageSizeFactor-> 2.0,
 printForceVal->False,
 printElementLabel->True
 printForceName->True
 legendForce-> \"automatic\" 
 verbose-> False 
  ";


plotHodograph::usage="plotHodograph[ v , adjacentE, options  ] 
 plots velocities of  all elements. 
 ARGUMETS :  
  v = list of   velocity vectors  of all elements  
  adjacentE  = list of pairs with adjacent element numbers left and right (looking along edgesT)  from getTopology[ ]
...
getHodograph::usage = " {v, Loads} = getHodograph[ verbose->True ]    
Input is taken from global variables 
OUTPUT: 
  v     = list of element velocities   calculated (created) in  getHodograph 
  Loads += contribution to  element forces  due to cohesion  and (symbolically) due to unknown interaction forces between neighbour elements.
OPTIONS:  
 verbose->True
 
 While entering getHodograph the list Loads contains external load force acting on each element. \
\n This  force is superposed in  getHodograph by the contributions from   neighbouring elements, i.e.  \
...
\n plottingPPF=Array[{}&,ne];
 ...
 \n Displays the resulting values of all non-zero forces acting on each element.
 ";
...
printForces[solution_] := PrintForces[solution]; 
