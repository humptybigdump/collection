...
def get_timeshift_specfem(geometry, sll_x, sll_y, sl_s, grdpts_x, grdpts_y):
    """
    Returns timeshift table for given array geometry
...
def get_spoint_edit(stream, stime, etime):
    """
    Calculates start and end offsets relative to stime and etime for each
    trace in stream in samples.
    ...
    epoint = np.empty(len(stream), dtype=np.int32, order="C")
    for i, tr in enumerate(stream):
        if abs(tr.stats.starttime - stime) > 2*stream[0].stats.delta:
            msg = "Specified stime %s is smaller than starttime %s in stream"
            raise ValueError(msg % (stime, tr.stats.starttime))
        if abs(tr.stats.endtime - etime) > 2*stream[0].stats.delta:
            msg = "Specified etime %s is bigger than endtime %s in stream"
            raise ValueError(msg % (etime, tr.stats.endtime))
        # now we have to adjust to the beginning of real start time
        spoint[i] = int((stime - tr.stats.starttime) *
...
def array_processing_edit(stream, win_len, win_frac, sll_x, slm_x, sll_y, slm_y,
                     sl_s, semb_thres, vel_thres, frqlow, frqhigh, stime,
                     etime, prewhiten, verbose=False, coordsys='lonlat',
                     timestamp='mlabday', method=0, store=None, specfem=False, time_edit=False):
    """
    ...
    edit:   - include possibility to use code for specfem 2D model
            connect timestamp of claculated baz/s -value with center of window, not beginning
            - include possibility to process data, if the fraction of second is due to trim slightly different
    """
    res = []
    eotr = True
    ...
    # offset of arrays
    if time_edit:
        spoint, _epoint = get_spoint_edit(stream, stime, etime)
    else:
    ...
    newstart = stime
    #print(newstart)
    mean_of_window=newstart+win_len/2
    ...
    while eotr:
        ...
        if relpow > semb_thres and 1. / slow > vel_thres:
            res.append(np.array([mean_of_window.timestamp, relpow, abspow, baz,
                                 slow]))
            if verbose:
