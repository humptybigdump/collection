...
par(mfrow = c(2,4))
# Plotting average of forecast CDFs and empirical CDF (differences are hard to see!)
for(i in 1:8){
...
for(i in 1:8){
  assign(x = paste0("Fn",i), value = Fn(get(paste0("mu",i)),get(paste0("sd",i))))
  plot(x = function(x) get(paste0("Fn",i))(x) - FnY(x), col = "red", main = paste0("F",i),xlim = c(-7,7),ylim = c(-0.1,0.1))
