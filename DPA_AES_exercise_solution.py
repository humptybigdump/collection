...
#Helper Function to get the ith-bit of value. 
...
#The actual key
known_key_hex = [0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c]
known_key_decimal = [43, 126, 21, 22, 40, 174, 210, 166, 171, 247, 21, 136, 9, 207, 79, 60]
...
for guess in range (1,256):
    for i in range(2000):   
        hypothetical_leakage = aes_leakage_guess(guess, mesg_ar[i][5], 6)
        if(hypothetical_leakage == 0):
            traces_0.append(traces[i])
        else:
            traces_1.append(traces[i])
                    
    mean_trace_0 = mean(traces_0)
    mean_trace_1 = mean(traces_1)
    mean_diff = mean_trace_0 - mean_trace_1
