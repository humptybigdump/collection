...
(*---------------------------------------------usages--------------------------------------------------------------------*)
  ...
  RunVHPTest::usage = "RunVHPTest[ Mat_,ICond_,Loading_ ]  
   INPUT: Mat   = {\[Kappa]B, \[Lambda]B, \[Phi], Iv, ee0, Pe0, evisPref} 
...
getElasticIsomorph::usage = "getElasticIsomorph[P_,Q_,extendedMat_]  
   ...
   OUTPUT:  
   OPTIONS: 
returns the isotropic hypoelastic stiffness for the isomorphic P-Q-space   ( barotropic ).   
";
...
controlPanelVHP::usage = "controlVHP[ ] 
   INPUT: via global variable 
      Mat =  {\[Kappa]B, \[Lambda]B, \[Phi], Iv, ee0, Pe0, evisPref};
...
RunVHPTest[Mat_,ICond_,Loading_]:=Module[  { ntime, d\[Epsilon], dT, dt,  directions, dirTrial ,  \[Kappa]B,\[Lambda]B,\[Phi],Iv,ec0,pc0,evisPref, MC,ME,a, \[Beta]b, extendedMat,
